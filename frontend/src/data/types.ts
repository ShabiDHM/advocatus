...
// PHOENIX PROTOCOL - TYPES V9.0 (SUBSCRIPTION MATRIX SYNC)
// 1. IMPORTED: New enums for AccountType, SubscriptionTier, ProductPlan.
// 2. UPDATED: User and UpdateUserRequest interfaces now include the new subscription fields.
// 3. DEPRECATED: Old 'plan_tier' string is now superseded by the new enum-based fields.
...
export interface User { 
    ...
    // --- PHOENIX V9.0: New Subscription Matrix ---
...
// --- PHOENIX V9.0: Updated with new fields ---
...
export interface SubscriptionUpdate {
    ...
    plan_tier?: string; // Legacy, prefer new fields in UpdateUserRequest
}
...
export interface CaseAnalysisResult { summary?: string; key_issues?: string[]; legal_basis?: string[]; strategic_analysis?: string; weaknesses?: string[]; action_plan?: string[]; risk_level?: string; red_flags?: string[]; contradictions?: string[]; chronology?: ChronologyEvent[]; judicial_observation?: string; strategic_summary?: string; error?: string; }
export interface SpreadsheetAnalysisResult { file_id?: string; filename: string; record_count: number; columns: string[]; narrative_report: string; charts: any[]; anomalies: any[]; key_statistics: Record<string, string | number>; preview_rows?: Record<string, any>[]; processed_at: string; }
export interface Organization { id: string; name: string; owner_id: string; tier: 'TIER_1' | 'TIER_2'; plan: string; status: string; expiry?: string; seat_limit: number; seat_count: number; created_at: string; owner_email?: string; }