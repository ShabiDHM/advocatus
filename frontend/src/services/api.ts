...
// PHOENIX PROTOCOL - API MASTER V7.3 (FEATURE 4: DEPOSITION)
// 1. ADDED: analyzeDeposition method.
// 2. STATUS: Fully supporting all analysis modes.
...
import type {
    ...
    SpreadsheetAnalysisResult, DepositionAnalysisResult
} from '../data/types';

// ... (Rest of existing interfaces: AuditIssue, TaxCalculation, WizardState, InvoiceUpdate, etc.)
...
class ApiService {
    ...
    // Feature 4: Deposition (NEW)
    public async analyzeDeposition(caseId: string, file: File): Promise<DepositionAnalysisResult> {
        const formData = new FormData();
        formData.append('file', file);
        const response = await this.axiosInstance.post<DepositionAnalysisResult>(`/cases/${caseId}/analyze/deposition`, formData);
        return response.data;
    }
