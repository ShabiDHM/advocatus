...
// PHOENIX PROTOCOL - DUAL FORENSIC AUTOPSY LAB V7.0 (GRANULAR ACTIVE TAB PURGE & 0MS PERSISTENCE)
// ZERO TS WARNINGS • SINGLE-TAB ADMIN TRASH PURGE • PRESERVES OTHER PILLARS • 100% COMPLETE CODE
...
export const DocumentForensicLab: React.FC<DocumentForensicLabProps> = ({
  ...
  // PHOENIX FIX: KOSHI FSHIN VETËM SHTJELLËN AKTIVE NGA MONGODB (PRESERVES OTHER PILLARS!)
  const handleAdminPurgeSinglePillar = async () => {
    ...
    try {
      if (autopsyScope === 'DOCUMENT') {
        ...
        // Fshin vetëm shtjellën aktive duke dërguar tekst bosh në MongoDB
        await forensicService.saveDocumentPillar(caseId, selectedDocId, activePillar, "");
        ...
        // Fshin vetëm shtjellën aktive të lëndës në MongoDB
  ...
  // GJENERIMI ME STREAM DHE RUAJTJA NË MONGODB
      ...
      <div className={`${isFullscreen ? 'lg:col-span-12' : 'lg:col-span-7'} glass-panel p-5 sm:p-6 rounded-3xl border border-main bg-card shadow-sm space-y-4 flex flex-col justify-between transition-all duration-300 relative`}>
        <div className="space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-main pb-3.5">
            ...
            {/* Butonat e Veprimit me Koshin Granular për Shtjellën Aktive */}
