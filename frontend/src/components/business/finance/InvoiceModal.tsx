...
// PHOENIX PROTOCOL - INVOICE MODAL V6.0 (EXECUTIVE DESIGN SYSTEM)
...
export const InvoiceModal: React.FC<InvoiceModalProps> = ({ isOpen, onClose, onSuccess, cases, editingInvoice }) => {
    ...
    return (
        <div className="fixed inset-0 bg-canvas/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="glass-panel border border-main w-full max-w-2xl max-h-[90vh] overflow-y-auto p-8 rounded-3xl animate-in fade-in zoom-in-95 duration-200 custom-finance-scroll">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-text-primary">{editingInvoice ? t('finance.editInvoice') : t('finance.createInvoice')}</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <select 
                                ...
                                className="glass-input w-full px-4 py-2.5 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40"
                            >
                                <option value="">-- {t('finance.noCase', 'Pa Lëndë')} --</option>
                                {cases.map(c => <option key={c.id} value={c.id} className="bg-canvas text-text-primary">{c.title}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="block text-xs text-text-secondary mb-1 font-bold uppercase">{t('business.clientName', 'Emri')}</label>
                            <input 
                                required type="text" 
                                className="glass-input w-full px-4 py-2.5 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs text-text-secondary mb-1 font-bold uppercase">{t('business.publicEmail')}</label>
                                <input 
                                    type="email" 
                                    className="glass-input w-full px-4 py-2.5 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                            ...
                            <div>
                                <label className="block text-xs text-text-secondary mb-1 font-bold uppercase">{t('business.phone')}</label>
                                <input 
                                    type="text" 
                                    className="glass-input w-full px-4 py-2.5 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs text-text-secondary mb-1 font-bold uppercase">{t('business.city')}</label>
                                <input 
                                    type="text" 
                                    className="glass-input w-full px-4 py-2.5 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                            ...
                            <div>
                                <label className="block text-xs text-text-secondary mb-1 font-bold uppercase">{t('business.taxId')}</label>
                                <input 
                                    type="text" 
                                    className="glass-input w-full px-4 py-2.5 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                        ...
                        <div>
                            <label className="block text-xs text-text-secondary mb-1 font-bold uppercase">{t('business.address')}</label>
                            <input 
                                type="text" 
                                className="glass-input w-full px-4 py-2.5 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                        ...
                        <div className="flex items-center gap-3 bg-surface/20 p-3 rounded-xl border border-main">
                            <input 
                                type="checkbox" 
                                id="vatToggle" 
                                checked={includeVat} 
                                onChange={(e) => setIncludeVat(e.target.checked)} 
                                className="w-4 h-4 text-primary-start rounded border-main focus:ring-primary-start" 
                            />
                            <label htmlFor="vatToggle" className="text-sm text-text-secondary cursor-pointer select-none">
                                Apliko TVSH (18%)
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-6 border-t border-main">
                        ...
                        {lineItems.map((item, index) => (
                            <div key={index} className="flex flex-col sm:flex-row gap-2 items-center">
                                <input 
                                    ...
                                    className="flex-1 w-full glass-input px-3 py-2 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                                ...
                                <input 
                                    ...
                                    className="w-full sm:w-20 glass-input px-3 py-2 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                                    value={item.quantity} 
                                ...
                                <input 
                                    ...
                                    className="w-full sm:w-24 glass-input px-3 py-2 rounded-xl border border-main bg-surface focus:border-primary-start focus:ring-1 focus:ring-primary-start/40" 
                                ...
                                <button 
                                    ...
                                    className="p-2 text-danger-start hover:bg-danger-start/10 rounded-lg self-end sm:self-center"
                                >
                                    <Trash2 size={18} />
                                </button>
                            </div>
                        ...
                        <button 
                            type="button" 
                            onClick={addLineItem} 
                            className="text-sm text-primary-start hover:underline flex items-center gap-1 font-medium"
                        >
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        <button 
                            type="button" 
                            onClick={onClose} 
                            className="px-6 py-2.5 rounded-xl text-text-secondary hover:text-text-primary hover:bg-surface/30 transition-colors"
                        >
                            {t('general.cancel')}
                        </button>
                        <button 
                            type="submit" 
                            disabled={loading} 
                            className="btn-primary px-8 py-2.5 rounded-xl font-bold shadow-lg hover:scale-[1.02] transition-transform flex items-center gap-2"
                        >
