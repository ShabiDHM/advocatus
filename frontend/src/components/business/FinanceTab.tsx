...
const TabButton = ({ label, icon, isActive, onClick }: { label: string, icon: React.ReactNode, isActive: boolean, onClick: () => void }) => (
    <button onClick={onClick} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ${isActive ? 'bg-secondary-start/10 text-secondary-start' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
        {icon} {label}
    </button>
);
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="lg:col-span-2 bg-background-dark/50 border border-glass-edge rounded-3xl p-6 flex flex-col min-h-[600px]">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 border-b border-white/10 pb-4"><h2 className="text-lg font-bold text-white shrink-0">{t('finance.activityAndReports')}</h2><div className="w-full sm:w-auto flex items-center gap-2 bg-background-light p-1 rounded-xl border border-white/5"><TabButton label={t('finance.tabTransactions')} icon={<Activity size={16} />} isActive={activeTab === 'transactions'} onClick={() => setActiveTab('transactions')} /><TabButton label={t('finance.tabReports')} icon={<BarChart2 size={16} />} isActive={activeTab === 'reports'} onClick={() => setActiveTab('reports')} /><TabButton label={t('finance.tabHistory')} icon={<History size={16} />} isActive={activeTab === 'imports'} onClick={() => setActiveTab('imports')} /></div></div>
                    <div className="flex-1 overflow-y-auto custom-finance-scroll -mr-2 pr-2">
                        ...
                        {activeTab === 'imports' && (<div className="space-y-3"><p className="text-sm text-gray-400 mb-4">{t('finance.historyDescription')}</p></div>)}
                    </div>
                </div>
            </div>
