...
const TabButton = ({ label, icon, isActive, onClick }: { label: string, icon: React.ReactNode, isActive: boolean, onClick: () => void }) => (
    <button onClick={onClick} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ${isActive ? 'bg-secondary-start/10 text-secondary-start' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
        {icon} {label}
    </button>
);
...
export const FinanceTab: React.FC = () => {
    ...
    const handleFileUpload = async () => { if (!uploadFile) return; setIsUploading(true); setUploadError(null); try { /* const response = await apiService.uploadPosFile(uploadFile); */ alert("Upload functionality is being connected to the backend."); closeImportModal(); } catch (error: any) { setUploadError(error.message || "Failed to upload file."); } finally { setIsUploading(false); } };
