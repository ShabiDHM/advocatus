...
import { 
    ...
    Landmark, Zap, Wifi, Receipt, Utensils
} from 'lucide-react';
...
export const FinanceTab: React.FC = () => {
    ...
    const handleViewExpense = async (expense: Expense) => { 
        ...
        try { 
            ...
            if (expense.receipt_url) {
                ...
                blob = response.blob; filename = response.filename;
                if (blob.type === 'application/json') throw new Error('Invalid receipt data');
            } else {
                const file = generateDigitalReceipt(expense); blob = file; filename = file.name;
            }
            const url = window.URL.createObjectURL(blob); 
            setViewingUrl(url); 
            setViewingDoc({ id: expense.id, file_name: filename, mime_type: blob.type, status: 'READY' } as any); 
    ...
    const submitArchiveExpense = async () => { 
        ...
        try { 
            ...
            await apiService.uploadArchiveItem(fileToUpload, fileToUpload.name, "EXPENSE", selectedCaseForArchive || undefined, undefined); 
            alert(t('general.saveSuccess')); setShowArchiveExpenseModal(false); setSelectedCaseForArchive(""); 
        } catch { alert(t('error.generic')); } 
    };
    ...
    return (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                    ...
                    <div className="flex-1 flex flex-col min-h-0 relative -mr-2 pr-2 overflow-hidden">
                        ...
                        {activeTab === 'reports' && (
                            <div className="h-full overflow-y-auto custom-finance-scroll pr-2 space-y-6">
                                {!analyticsData ? <div className="space-y-6"><SkeletonChart /><SkeletonGrid /></div> : <FinanceAnalytics data={analyticsData} />}
                            </div>
                        )}
                                        ...
                                        filteredHistory.map((item) => (
                                            <div key={item.caseData.id} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                                                <div className="p-4 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors" onClick={() => setExpandedCaseId(expandedCaseId === item.caseData.id ? null : item.caseData.id)}>
                                                    <div className="flex items-center gap-3"><div className="p-2 bg-blue-500/20 text-blue-400 rounded-lg"><Briefcase size={18} /></div><div><h4 className="font-bold text-white text-sm">{item.caseData.title}</h4><p className="text-xs text-gray-500">{item.caseData.case_number}</p></div></div>
            ...
            {/* ARCHIVE INVOICE MODAL (Kept inline as it's tiny, or could be extracted similarly) */}
