...
// PHOENIX PROTOCOL - FINANCE TAB V15.0 (SEARCH & SCROLL)
// 1. ADDED: Search functionality for transactions (Client, Category, Amount, ID).
// 2. CHANGED: Transaction list height restricted to ~4 items (380px) to force scrolling.
// 3. UI: Added Search Input field above the list.
...
import { 
    ...
    Edit2, Eye, Download, Archive, Trash2, CheckCircle, Paperclip, X, User, Activity, Loader2, UploadCloud, BarChart2, History, FileUp, MoreVertical, ChevronRight, ArrowRight, Table, Search
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { 
    apiService, Expense, ExpenseCreateRequest, ImportBatchOut, TransactionOut, 
    ImportMappingResponse, AnalyticsDashboardData 
} from '../../services/api';
...
export const FinanceTab: React.FC = () => {
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return sortedTransactions.filter(tx => {
            if (tx.type === 'invoice') {
                return (
                    tx.client_name.toLowerCase().includes(lowerTerm) ||
                    tx.invoice_number?.toLowerCase().includes(lowerTerm) ||
                    tx.total_amount.toString().includes(lowerTerm)
                );
            } else {
                return (
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            if ('mapping_required' in response && response.mapping_required) {
                setMappingData(response as ImportMappingResponse);
                const initialMappings: Record<string, string> = {};
                (response as ImportMappingResponse).detected_headers?.forEach(h => initialMappings[h] = "");
    ...
    const handleMappingSubmit = async () => {
        if (!mappingData || !mappingData.upload_id) return;
        const mappedValues = Object.values(userMappings);
        if (!mappedValues.includes('product') || !mappedValues.includes('total')) {
            alert("Ju lutem caktoni fushat e detyrueshme: 'Produkti' dhe 'Totali'.");
            return;
        }

        setIsUploading(true);
        try {
            const batch = await apiService.finalizePosImport(mappingData.upload_id, userMappings);
            setImportBatches(prev => [batch, ...prev].sort((a,b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()));
            const analytics = await apiService.getAnalyticsDashboard(30);
            setAnalyticsData(analytics);
            alert(`Sukses! U importuan ${batch.row_count} transaksione.`);
            ...
            setUploadFile(null);
             ...
             const detail = error.response?.data?.detail || "Dështoi mapimi i kolonave.";
             alert(detail);
        } finally {
            setIsUploading(false);
        }
    ...
    const handleMappingChange = (header: string, systemField: string) => {
        setUserMappings(prev => ({ ...prev, [header]: systemField }));
    };
    ...
    const translateSystemField = (field: string) => {
        const map: Record<string, string> = {
            'product': 'Produkti (E detyrueshme)',
            'total': 'Totali (E detyrueshme)',
            'date': 'Data',
            'quantity': 'Sasia',
            'price': 'Çmimi për njësi',
            'ref': 'Referenca / Nr. Faturës'
        };
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="lg:col-span-1 flex flex-col gap-6">
                    ...
                    <div className="bg-background-dark/50 border border-glass-edge rounded-3xl p-6 space-y-3">
                        ...
                        <QuickActionButton icon={<FileUp size={18} />} label={t('finance.importTransactions')} onClick={() => setShowImportModal(true)} color="text-indigo-400" />
                ...
                <div className="lg:col-span-2 bg-background-dark/50 border border-glass-edge rounded-3xl p-6 flex flex-col min-h-[600px]">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 border-b border-white/10 pb-4">
                        ...
                        <div className="w-full sm:w-auto flex items-center gap-2 bg-background-light p-1 rounded-xl border border-white/5 overflow-x-auto no-scrollbar">
                            ...
                            <TabButton label={t('finance.tabHistory')} icon={<History size={16} />} isActive={activeTab === 'imports'} onClick={() => setActiveTab('imports')} />
                        </div>
                    </div>
                        ...
                        {activeTab === 'transactions' && (
                            <div className="space-y-4">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        placeholder={t('header.searchPlaceholder') || "Kërko transaksione..."}
                                        className="block w-full pl-10 pr-3 py-2 border border-white/10 rounded-xl leading-5 bg-white/5 text-gray-300 placeholder-gray-500 focus:outline-none focus:bg-white/10 focus:border-indigo-500 sm:text-sm transition-all"
                                        value={searchTerm}
                                        onChange={(e) => setSearchTerm(e.target.value)}
                                    />
                                </div>

                                {/* Transaction List (Restricted Height) */}
                                <div className="space-y-3 h-[380px] overflow-y-auto custom-finance-scroll pr-2">
                                    {filteredTransactions.length === 0 ? (
                                        <p className="text-gray-500 italic text-sm text-center py-10">{t('finance.noTransactions')}</p>
                                    ) : (
                                                        ...
                                                        <tbody className="divide-y divide-white/5">
                                                            {analyticsData.top_products.map((p, i) => (
                                                                <tr key={i} className="hover:bg-white/5">
                                                                    <td className="px-2 py-2 font-medium text-white truncate max-w-[120px]" title={p.product_name}>{p.product_name}</td>
                                                                    <td className="px-2 py-2 text-right font-mono">{p.total_quantity}</td>
                                                                    <td className="px-2 py-2 text-right font-bold text-emerald-400">€{p.total_revenue.toFixed(2)}</td>
                                                                </tr>
                                                            ))}
                                                        </tbody>
                                                    </table>
            ...
            {showMappingModal && mappingData && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-background-dark border border-glass-edge rounded-2xl w-full max-w-2xl p-6 flex flex-col max-h-[90vh]">
                        <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
                            <h2 className="text-xl font-bold text-white flex items-center gap-2">
                                <Table size={20} className="text-amber-400" /> 
                                Lidhja e Kolonave (Smart Map)
                            </h2>
                            <button onClick={() => setShowMappingModal(false)} className="text-gray-400 hover:text-white"><X size={24} /></button>
                        </div>
                        
                        <div className="flex-1 overflow-y-auto custom-finance-scroll mb-6">
                            <p className="text-gray-400 text-sm mb-4">
                                Sistemit i duhet ndihmë për të kuptuar kolonat e skedarit tuaj. Ju lutem lidhni kolonat e gjetura me fushat e sistemit.
                                Kjo lidhje do të ruhet për herën tjetër.
                            </p>
                            
                            <div className="space-y-4">
                                {mappingData.detected_headers?.map((header) => (
                                    <div key={header} className="grid grid-cols-1 sm:grid-cols-7 gap-4 items-center bg-white/5 p-3 rounded-lg border border-white/10">
                                        <div className="sm:col-span-3">
                                            <label className="text-xs text-gray-500 block mb-1">Kolona në Skedar</label>
                                            <div className="font-mono text-sm text-white font-bold truncate" title={header}>{header}</div>
                                        </div>
                                        <div className="sm:col-span-1 flex justify-center text-gray-500">
                                            <ArrowRight size={16} />
                                        </div>
                                        <div className="sm:col-span-3">
                                            <label className="text-xs text-gray-500 block mb-1">Fusha në Sistem</label>
                                            <select 
                                                value={userMappings[header] || ""} 
                                                onChange={(e) => handleMappingChange(header, e.target.value)}
                                                className="w-full bg-background-light border-glass-edge rounded-md px-3 py-2 text-sm text-white focus:ring-2 focus:ring-indigo-500"
                                            >
                                                <option value="">-- Zgjidh --</option>
                                                {mappingData.system_fields?.map(field => (
                                                    <option key={field} value={field}>{translateSystemField(field)}</option>
                                                ))}
                                            </select>
                                        </div>
                                    </div>
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
                            <button onClick={() => setShowMappingModal(false)} className="px-4 py-2 text-gray-400 hover:bg-white/5 rounded-lg">Anulo</button>
                            <button 
            ...
            {/* Other Modals (Invoice, Expense, etc.) remain the same... */}
