...
const TabButton = ({ label, icon, isActive, onClick }: { label: string, icon: React.ReactNode, isActive: boolean, onClick: () => void }) => (
    <button onClick={onClick} className={`w-full sm:w-auto flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-2.5 rounded-xl text-[10px] sm:text-xs md:text-sm font-bold transition-all duration-300 ${isActive ? 'btn-primary shadow-lg' : 'text-text-secondary hover:text-text-primary hover:bg-hover'}`}>
...
const SkeletonChart = () => (
    <div className="card-panel rounded-2xl p-4 animate-pulse"><div className="h-6 bg-surface/30 rounded w-1/3 mb-4"></div><div className="h-64 bg-surface/20 rounded"></div></div>
);

const SkeletonGrid = () => (
     <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-pulse"><div className="card-panel rounded-2xl p-4"><div className="h-6 bg-surface/30 rounded w-1/2 mb-4"></div><div className="h-64 bg-surface/20 rounded"></div></div><div className="card-panel rounded-2xl p-4"><div className="h-6 bg-surface/30 rounded w-1/2 mb-4"></div><div className="space-y-2 mt-4"><div className="h-8 bg-surface/20 rounded"></div><div className="h-8 bg-surface/20 rounded"></div><div className="h-8 bg-surface/20 rounded"></div><div className="h-8 bg-surface/20 rounded"></div><div className="h-8 bg-surface/20 rounded"></div></div></div></div>
);
...
export const FinanceTab: React.FC = () => {
    ...
    return (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                        ...
                                        <div key={`${tx.type}-${tx.id}`} className="group relative card-panel rounded-2xl overflow-hidden hover-lift flex flex-col h-fit border-border-main">
                                            ...
                                            <div className="p-4 flex-1">
                                                <div className="flex items-start justify-between mb-3">
                                                    <div className={`p-2.5 rounded-xl ${tx.type === 'invoice' ? 'bg-status-success/10 text-status-success' : 'bg-status-danger/10 text-status-danger'}`}>{tx.type === 'invoice' ? <FileText size={20} /> : getCategoryIcon(tx.category)}</div>
                                                    <div className="text-right"><p className={`text-lg font-bold ${tx.type === 'invoice' ? 'text-status-success' : 'text-status-danger'}`}>{tx.type === 'invoice' ? '+' : '-'}€{tx.type === 'invoice' ? tx.total_amount.toFixed(2) : tx.amount.toFixed(2)}</p><span className="text-[10px] text-text-muted font-mono">{new Date(tx.date).toLocaleDateString()}</span></div>
                                                </div>
                                            ...
                                            <div className="border-t border-border-main p-2 flex items-center justify-end gap-1 bg-canvas/20">
                                                <button onClick={() => { if(tx.type === 'invoice') { setEditingInvoice(tx); setShowInvoiceModal(true); } else { setEditingExpense(tx); setShowExpenseModal(true); } }} className="p-2 hover:bg-hover rounded-lg text-warning-start transition-colors" title={t('general.edit')}><Edit2 size={14} /></button>
                                                <button onClick={() => tx.type === 'invoice' ? handleViewInvoice(tx) : handleViewExpense(tx)} disabled={openingDocId != null && openingDocId !== tx.id} className="p-2 hover:bg-hover rounded-lg text-primary-start transition-colors disabled:opacity-50" title={t('general.view')}>{openingDocId === tx.id ? <Loader2 size={14} className="animate-spin"/> : <Eye size={14} />}</button>
                                                <button onClick={() => tx.type === 'invoice' ? downloadInvoice(tx.id) : handleDownloadExpense(tx)} className="p-2 hover:bg-hover rounded-lg text-status-success transition-colors" title={t('general.download')}><Download size={14} /></button>
                                                <button onClick={() => { if (tx.type === 'invoice') { setSelectedInvoiceId(tx.id); setShowArchiveInvoiceModal(true); } else { setSelectedExpenseId(tx.id); setShowArchiveExpenseModal(true); } }} className="p-2 hover:bg-hover rounded-lg text-indigo-400 transition-colors" title={t('general.archive')}><Archive size={14} /></button>
                                                <button onClick={() => tx.type === 'invoice' ? deleteInvoice(tx.id) : deleteExpense(tx.id)} className="p-2 hover:bg-hover rounded-lg text-status-danger transition-colors" title={t('general.delete')}><Trash2 size={14} /></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
            ...
            {showArchiveInvoiceModal && (<div className="fixed inset-0 bg-canvas/80 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="glass-panel border border-border-main w-full max-w-md p-8 rounded-3xl animate-in fade-in zoom-in-95 duration-200"><h2 className="text-xl font-bold text-text-primary mb-6">{t('finance.archiveInvoice')}</h2><div className="mb-8"><label className="block text-xs text-text-muted mb-1 font-bold uppercase">{t('drafting.selectCaseLabel')}</label><select className="glass-input w-full px-4 py-2.5 rounded-xl" value={selectedCaseForArchive} onChange={(e) => setSelectedCaseForArchive(e.target.value)}><option value="">{t('archive.generalNoCase')}</option>{cases.map(c => (<option key={c.id} value={c.id} className="bg-canvas text-text-primary">{c.title}</option>))}</select></div><div className="flex justify-end gap-3"><button onClick={() => setShowArchiveInvoiceModal(false)} className="px-6 py-2.5 rounded-xl text-text-secondary hover:text-text-primary hover:bg-hover">{t('general.cancel')}</button><button onClick={submitArchiveInvoice} className="btn-primary px-8 py-2.5 rounded-xl font-bold shadow-lg">{t('general.save')}</button></div></div></div>)}
            {showArchiveExpenseModal && (<div className="fixed inset-0 bg-canvas/80 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="glass-panel border border-border-main w-full max-w-md p-8 rounded-3xl animate-in fade-in zoom-in-95 duration-200"><h2 className="text-xl font-bold text-text-primary mb-6">{t('finance.archiveExpenseTitle')}</h2><div className="mb-8"><label className="block text-xs text-text-muted mb-1 font-bold uppercase">{t('drafting.selectCaseLabel')}</label><select className="glass-input w-full px-4 py-2.5 rounded-xl" value={selectedCaseForArchive} onChange={(e) => setSelectedCaseForArchive(e.target.value)}><option value="">{t('archive.generalNoCase')}</option>{cases.map(c => (<option key={c.id} value={c.id} className="bg-canvas text-text-primary">{c.title}</option>))}</select></div><div className="flex justify-end gap-3"><button onClick={() => setShowArchiveExpenseModal(false)} className="px-6 py-2.5 rounded-xl text-text-secondary hover:text-text-primary hover:bg-hover">{t('general.cancel')}</button><button onClick={submitArchiveExpense} className="btn-primary px-8 py-2.5 rounded-xl font-bold shadow-lg">{t('general.save')}</button></div></div></div>)}
