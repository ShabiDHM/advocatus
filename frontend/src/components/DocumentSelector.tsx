...
// PHOENIX PROTOCOL - DOCUMENT SELECTOR V1.3 (SEMANTIC THEME VARIABLES)
// 1. REPLACED: hardcoded colors with semantic theme variables.
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        disabled={disabled}
        className="glass-input w-full h-12 md:h-11 rounded-xl flex items-center justify-between gap-2 px-4 py-2 text-sm font-medium text-text-primary hover:bg-surface/10 transition-colors disabled:opacity-50"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full left-0 mt-1 w-64 max-h-64 overflow-y-auto glass-panel border surface-border rounded-lg shadow-xl z-50 p-2"
          >
            {documents.length === 0 ? (
              <div className="text-xs text-text-secondary/70 p-2">Nuk ka dokumente</div>
            ) : (
              <>
                <div className="flex items-center gap-2 p-1 border-b surface-border mb-1">
                  <button
                    onClick={selectAll}
                ...
                {documents.map(doc => (
                  ...
                  >
                    <input
                      ...
                      className="rounded border-surface-border bg-canvas text-primary-start checked:bg-primary-start checked:border-transparent focus:ring-primary-start"
                    />
