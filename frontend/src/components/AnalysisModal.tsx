    ...
    if (typeof item === 'object' && item !== null && (item.law || item.title)) {
        ...
        return (
            <div className="flex flex-col gap-3 w-full">
                ...
                {body && (
                    <div className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed pl-5 border-l-2 border-border-main ml-0.5 mt-1">
            ...
            <AnimatePresence>
                {show && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute top-full left-1/2 transform -translate-x-1/2 mt-3 w-56 p-4 bg-white dark:bg-gray-800 text-xs font-medium text-gray-700 dark:text-gray-300 rounded-xl border border-border-main shadow-lawyer-dark z-[100] text-center leading-relaxed"
                    >
...
const AnalysisModal: React.FC<AnalysisModalProps> = ({ isOpen, onClose, result, caseId, isLoading = false }) => {
  ...
  const renderSubTabLoader = () => (
    <div className="flex-1 flex flex-col items-center justify-center text-center py-32">
        ...
        <h3 className="text-xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-3 mt-6">{t('analysis.loading_deep_title', 'Duke Simuluar...')}</h3>
        <p className="text-gray-500 dark:text-gray-400 text-xs font-bold uppercase tracking-widest">{t('analysis.rag_processing', 'Analiza e thellë statutore...')}</p>
    </div>
  );
        ...
        >
          ...
          {isLoading ? (
             <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-gray-900/50">
                 ...
                 <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-3 mt-6">{t('analysis.loading_title', 'Duke Analizuar...')}</h3>
                 <p className="text-gray-500 dark:text-gray-400 text-xs font-bold uppercase tracking-widest">Kjo mund të marrë disa sekonda</p>
             </div>
          ) : (
             <>
                {/* Tabs */}
                        ...
                        {activeTab === 'legal' && (
                            <>
                                <div className="bg-white dark:bg-gray-800 p-8 rounded-[1.5rem] border border-border-main shadow-sm hover-lift">
                                    <h3 className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-5 flex items-center gap-3">
                                    ...
                                    <div className="bg-primary-start/5 p-8 rounded-[1.5rem] border border-primary-start/20 shadow-sm hover-lift">
                                        ...
                                        <ul className="space-y-4">
                                            {legal_basis.map((lawItem: any, i: number) => {
                                                ...
                                                return (
                                                    <li key={i} className={`flex gap-4 text-sm items-start p-5 rounded-xl transition-colors shadow-sm bg-white dark:bg-gray-800 border ${isGlobal ? 'border-indigo-500/30' : 'border-border-main'}`}>
                        ...
                        {activeTab === 'war_room' && (
                            <div className="h-full flex flex-col">
                                ...
                                <div className="flex flex-wrap gap-3 mb-8 shrink-0 pb-2">
                                    <button onClick={() => setWarRoomSubTab('strategy')} className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'strategy' ? 'bg-primary-start text-white shadow-accent-glow' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'}`}>
                                        <Target size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_strategy', 'Plani Strategjik')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('adversarial')} className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'adversarial' ? 'bg-danger-start text-white shadow-lg shadow-danger-start/30' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-danger-start'}`}>
                                        <Skull size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_adversarial', 'Simulimi i Palës')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('timeline')} className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'timeline' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-indigo-500'}`}>
                                        <Clock size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_timeline', 'Kronologjia')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('contradictions')} className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'contradictions' ? 'bg-warning-start text-white shadow-lg shadow-warning-start/30' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-warning-start'}`}>
                                ...
                                <div className="space-y-8 animate-in fade-in">
                                    {warRoomSubTab === 'strategy' ? (
                                        <div className="space-y-8">
                                            <div className="bg-white dark:bg-gray-800 p-8 rounded-[1.5rem] border border-border-main shadow-sm hover-lift">
                                                <h3 className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-5 flex items-center gap-3"><Target size={16} className="text-primary-start"/> {t('analysis.section_analysis', 'Analiza Strategjike')}</h3>
