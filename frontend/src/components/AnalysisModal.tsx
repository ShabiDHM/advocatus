...
// PHOENIX PROTOCOL - ANALYSIS MODAL V14.8 (Reduced text size)
    ...
    if (typeof item === 'object' && item !== null && (item.law || item.title)) {
        ...
        return (
            <div className="flex flex-col gap-3 w-full">
                ...
                {body && (
                    <div className="text-gray-700 dark:text-gray-300 text-[11px] leading-relaxed pl-5 border-l-2 border-border-main ml-0.5 mt-1">
            ...
            <AnimatePresence>
                {show && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute top-full left-1/2 transform -translate-x-1/2 mt-3 w-56 p-4 bg-white dark:bg-gray-800 text-[11px] font-medium text-gray-700 dark:text-gray-300 rounded-xl border border-border-main shadow-lawyer-dark z-[100] text-center leading-relaxed"
                    >
...
const AnalysisModal: React.FC<AnalysisModalProps> = ({ isOpen, onClose, result, caseId, isLoading = false }) => {
  ...
  const renderSubTabLoader = () => (
    <div className="flex-1 flex flex-col items-center justify-center text-center py-32">
        ...
        <h3 className="text-base font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-3 mt-6">{t('analysis.loading_deep_title', 'Duke Simuluar...')}</h3>
        ...
        >
          ...
          {isLoading ? (
             ...
             <>
                ...
                <div className="flex border-b border-border-main px-8 bg-white dark:bg-gray-900 shrink-0 overflow-x-auto no-scrollbar gap-8">
                    <button onClick={() => setActiveTab('legal')} className={`py-4 text-[11px] font-black uppercase tracking-widest flex items-center gap-3 border-b-2 transition-all whitespace-nowrap ${activeTab === 'legal' ? 'border-primary-start text-primary-start' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'}`}>
                        <Scale size={16}/> {t('analysis.tab_legal', 'Analiza Ligjore')}
                    </button>
                    <button onClick={handleWarRoomEntry} className={`py-4 text-[11px] font-black uppercase tracking-widest flex items-center gap-3 border-b-2 transition-all whitespace-nowrap ${activeTab === 'war_room' ? 'border-danger-start text-danger-start' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-danger-start'}`}>
                ...
                <div 
                  ...
                  style={{ fontSize: zoomLevel === 'large' ? '0.9375rem' : '0.8125rem' }}
                >
                                    ...
                                    <div className="bg-primary-start/5 p-8 rounded-[1.5rem] border border-primary-start/20 shadow-sm hover-lift">
                                        ...
                                        <ul className="space-y-4">
                                            {legal_basis.map((lawItem: any, i: number) => {
                                                ...
                                                return (
                                                    <li key={i} className={`flex gap-4 text-[11px] items-start p-5 rounded-xl transition-colors shadow-sm bg-white dark:bg-gray-800 border ${isGlobal ? 'border-indigo-500/30' : 'border-border-main'}`}>
                        ...
                        {activeTab === 'war_room' && (
                            <div className="h-full flex flex-col">
                                ...
                                <div className="flex flex-wrap gap-3 mb-8 shrink-0 pb-2">
                                    <button onClick={() => setWarRoomSubTab('strategy')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'strategy' ? 'bg-primary-start text-white shadow-accent-glow' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'}`}>
                                        <Target size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_strategy', 'Plani Strategjik')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('adversarial')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'adversarial' ? 'bg-danger-start text-white shadow-lg shadow-danger-start/30' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-danger-start'}`}>
                                        <Skull size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_adversarial', 'Simulimi i Palës')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('timeline')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'timeline' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-indigo-500'}`}>
                                        <Clock size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_timeline', 'Kronologjia')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('contradictions')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${warRoomSubTab === 'contradictions' ? 'bg-warning-start text-white shadow-lg shadow-warning-start/30' : 'bg-white dark:bg-gray-800 border border-border-main text-gray-600 dark:text-gray-400 hover:text-warning-start'}`}>
                                                ...
                                                <div className="space-y-4">
                                                    {action_plan.map((step: any, i: number) => (
                                                        <div key={i} className="flex items-start gap-5 text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-5 rounded-xl border border-success-start/10 shadow-sm">
                                                            <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-success-start/20 text-success-start font-black text-[11px] shrink-0">{i + 1}</span>
                                            ...
                                            <div className="grid gap-6">
                                                {deepResult.contradictions.length === 0 ? (
                                                    <div className="bg-white dark:bg-gray-800 p-12 rounded-[1.5rem] text-center border border-border-main shadow-sm">
                                                        ...
                                                        <p className="text-gray-900 dark:text-gray-100 font-bold text-base">{t('analysis.no_contradictions', 'Gjithçka e pastër.')}</p>
                                                        <p className="text-gray-500 dark:text-gray-400 text-[11px] mt-2">Nuk u gjetën kontradikta mes deklaratave dhe provave.</p>
                                                    </div>
                                                ) : (
                                                    deepResult.contradictions.map((c: Contradiction, i: number) => (
                                                        <div key={i} className="bg-white dark:bg-gray-800 border border-warning-start/30 p-6 rounded-[1.5rem] shadow-lg shadow-warning-start/5">
                                                            <div className="flex justify-between items-start mb-6 pb-4 border-b border-border-main">
                                                                <div className="flex items-center gap-3 text-warning-start font-black text-[10px] uppercase tracking-widest"><AlertOctagon size={16}/> {t('analysis.contradiction_label', 'Mospërputhje Factual')}</div>
          ...
          <div className="px-8 py-5 border-t border-border-main bg-gray-50 dark:bg-gray-800 flex flex-col sm:flex-row gap-4 justify-between items-center shrink-0">
              <button 
                  ...
                  className={`w-full sm:w-auto px-6 py-3.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all flex items-center justify-center gap-3 border ${
              ...
              <button onClick={onClose} className="btn-primary w-full sm:w-auto px-10 py-3.5 text-[10px] uppercase tracking-widest font-black">
