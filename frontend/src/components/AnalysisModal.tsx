...
// PHOENIX PROTOCOL - ANALYSIS MODAL V13.9 (Theme‑aware with explicit dark mode classes)
...
const renderCitationItem = (item: any) => {
    if (typeof item === 'object' && item !== null && (item.law || item.title)) {
        ...
        return (
            <div className="flex flex-col gap-3 w-full">
                ...
                {body && (
                    <div className="text-gray-700 dark:text-gray-300 text-sm whitespace-pre-wrap leading-relaxed pl-5 border-l-2 border-gray-200 dark:border-gray-700 ml-0.5 mt-1">
    ...
    return (
        <span className="leading-relaxed whitespace-pre-wrap text-gray-900 dark:text-gray-100">
            {parts.map((part, i) => {
            ...
            <AnimatePresence>
                {show && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute top-full left-1/2 transform -translate-x-1/2 mt-3 w-56 p-4 bg-white dark:bg-gray-800 text-[11px] font-medium text-gray-700 dark:text-gray-300 rounded-xl border border-gray-200 dark:border-gray-700 shadow-lg z-[100] text-center leading-relaxed"
                    >
...
const AnalysisModal: React.FC<AnalysisModalProps> = ({ isOpen, onClose, result, caseId, isLoading }) => {
  ...
  const renderRiskBadge = (level: string) => {
      ...
      let styles = 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20';
  ...
  const renderSubTabLoader = () => (
    <div className="flex-1 flex flex-col items-center justify-center text-center py-32">
        ...
        <h3 className="text-xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-3">{t('analysis.loading_deep_title', 'Duke Simuluar...')}</h3>
        <p className="text-gray-500 dark:text-gray-400 text-[11px] font-bold uppercase tracking-widest">{t('analysis.rag_processing', 'Analiza e thellë statutore...')}</p>
    </div>
  );
  ...
  const modalContent = (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4 sm:p-6" onClick={onClose}>
        <motion.div initial={{ scale: 0.98, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.98, opacity: 0, y: 10 }} className="w-full h-full sm:h-[90vh] sm:max-w-6xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-xl dark:shadow-2xl overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
          
          {/* Header */}
          <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50 shrink-0">
            <h2 className="flex items-center gap-4 min-w-0">
              ...
              <div className="flex flex-col gap-1">
                  <span className="text-xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-tighter">{t('analysis.title', 'Strategjia Ligjore')}</span>
                  <div className="hidden sm:flex items-center mt-1">{renderRiskBadge(risk_level)} {renderSuccessBadge(success_probability)}</div>
              </div>
            </h2>
            <button onClick={onClose} className="p-3 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-all shrink-0 border border-transparent hover:border-gray-200 dark:hover:border-gray-700"><X size={24} /></button>
          </div>
          
          <div className="sm:hidden px-6 py-4 bg-gray-50 dark:bg-gray-800/30 border-b border-gray-200 dark:border-gray-800 flex gap-2">
          ...
          {isLoading ? (
             <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-white dark:bg-gray-900">
                 ...
                 <h3 className="text-2xl font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest mb-3">{t('analysis.loading_title', 'Duke Analizuar...')}</h3>
                 <p className="text-gray-500 dark:text-gray-400 text-[11px] font-bold uppercase tracking-widest">Kjo mund të marrë disa sekonda</p>
             </div>
          ) : (
             <>
                {/* Tabs */}
                <div className="flex border-b border-gray-200 dark:border-gray-800 px-8 bg-white dark:bg-gray-900 shrink-0 overflow-x-auto no-scrollbar gap-8">
                    ...
                    <button onClick={handleWarRoomEntry} className={`py-4 text-xs font-black uppercase tracking-widest flex items-center gap-3 border-b-2 transition-all whitespace-nowrap ${activeTab === 'war_room' ? 'border-danger-start text-danger-start' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-danger-start'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
                    <div className="max-w-4xl mx-auto space-y-8">
                        {activeTab === 'legal' && (
                            <>
                                <div className="bg-white dark:bg-gray-800 p-8 rounded-[1.5rem] border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-lg hover-lift">
                                    <h3 className="text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-5 flex items-center gap-3">
                                        <Info size={16} className="text-primary-start"/> {t('analysis.section_summary', 'Përmbledhja e Rastit')}
                                    </h3>
                                    <div className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed border-l-2 border-primary-start/30 pl-5 ml-1">{renderCitationItem(summary)}</div>
                                </div>

                                {burden_of_proof && (
                                    <div className="bg-gray-50 dark:bg-gray-800/50 p-8 rounded-[1.5rem] border border-gray-200 dark:border-gray-700 shadow-sm hover-lift">
                                        <h3 className="text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-5 flex items-center gap-3">
                                            <Gavel size={16} className="text-primary-start"/> {t('analysis.section_burden', 'Barra e Provës')}
                                        </h3>
                                        <div className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed italic border-l-2 border-gray-200 dark:border-gray-700 pl-5 ml-1">{renderCitationItem(burden_of_proof)}</div>
                                    </div>
                                )}

                                {missing_evidence && missing_evidence.length > 0 && (
                                    <div className="bg-red-500/5 p-8 rounded-[1.5rem] border border-red-500/20 shadow-sm hover-lift">
                                        ...
                                        <div className="grid gap-3">
                                            {missing_evidence.map((item, idx) => (
                                                <div key={idx} className="flex items-center gap-4 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-4 rounded-xl border border-red-500/10 shadow-sm">
                                    ...
                                    <div className="bg-white dark:bg-gray-800 p-8 rounded-[1.5rem] border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-lg hover-lift">
                                        ...
                                        <div className="grid gap-4">
                                            {key_issues.map((issue: any, idx: number) => (
                                                <div key={idx} className="flex items-start gap-4 bg-gray-50 dark:bg-gray-900/30 p-5 rounded-xl border border-gray-200 dark:border-gray-700">
                                                    <span className="text-primary-start font-black text-lg leading-none opacity-50 mt-0.5">#{idx + 1}</span>
                                                    <div className="text-sm text-gray-700 dark:text-gray-300 font-medium leading-relaxed">{renderCitationItem(issue)}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-primary-start/5 p-8 rounded-[1.5rem] border border-primary-start/20 shadow-sm hover-lift">
                                        ...
                                        <ul className="space-y-4">
                                            {legal_basis.map((lawItem: any, i: number) => {
                                                ...
                                                return (
                                                    <li key={i} className={`flex gap-4 text-sm items-start p-5 rounded-xl transition-colors shadow-sm bg-white dark:bg-gray-800 border ${isGlobal ? 'border-indigo-500/30' : 'border-gray-200 dark:border-gray-700'}`}>
                        ...
                        {activeTab === 'war_room' && (
                            <div className="h-full flex flex-col">
                                ...
                                <div className="flex flex-wrap gap-3 mb-8 shrink-0 pb-2">
                                    <button onClick={() => setWarRoomSubTab('strategy')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'strategy' ? 'bg-primary-start text-white shadow-accent-glow' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'}`}>
                                        <Target size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_strategy', 'Plani Strategjik')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('adversarial')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'adversarial' ? 'bg-danger-start text-white shadow-lg shadow-danger-start/30' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:text-danger-start'}`}>
                                        <Skull size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_adversarial', 'Simulimi i Palës')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('timeline')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'timeline' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:text-indigo-500'}`}>
                                        <Clock size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_timeline', 'Kronologjia')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('contradictions')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'contradictions' ? 'bg-warning-start text-white shadow-lg shadow-warning-start/30' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:text-warning-start'}`}>
                                ...
                                <div className="space-y-8 animate-in fade-in">
                                    {warRoomSubTab === 'strategy' ? (
                                        <div className="space-y-8">
                                            <div className="bg-white dark:bg-gray-800 p-8 rounded-[1.5rem] border border-gray-200 dark:border-gray-700 shadow-sm dark:shadow-lg">
                                                <h3 className="text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-5 flex items-center gap-3"><Target size={16} className="text-primary-start"/> {t('analysis.section_analysis', 'Analiza Strategjike')}</h3>
                                                <div className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed border-l-2 border-primary-start/30 pl-5 ml-1">{renderCitationItem(strategic_analysis)}</div>
                                            </div>
                                            <div className="bg-red-500/5 p-8 rounded-[1.5rem] border border-red-500/20 shadow-sm">
                                                ...
                                                <ul className="space-y-3">
                                                    {weaknesses.map((w: any, i: number) => (
                                                        <li key={i} className="flex items-center gap-4 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-4 rounded-xl border border-red-500/10 shadow-sm">
                                            ...
                                            <div className="bg-green-500/5 p-8 rounded-[1.5rem] border border-green-500/20 shadow-sm">
                                                ...
                                                <div className="space-y-4">
                                                    {action_plan.map((step: any, i: number) => (
                                                        <div key={i} className="flex items-start gap-5 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-5 rounded-xl border border-green-500/10 shadow-sm">
                                        ...
                                        isSimLoading ? renderSubTabLoader() : deepResult?.adversarial_simulation ? (
                                            <div className="space-y-8">
                                                <div className="bg-white dark:bg-gray-800 p-8 rounded-[1.5rem] border border-danger-start/30 shadow-lg shadow-danger-start/5">
                                                    ...
                                                    <div className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed font-medium">{renderCitationItem(deepResult.adversarial_simulation.opponent_strategy)}</div>
                                                </div>
                                                <div className="grid gap-4">
                                                    {deepResult.adversarial_simulation.weakness_attacks.map((attack: string, i: number) => (
                                                        <div key={i} className="flex gap-4 bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                                                            <Target size={18} className="text-danger-start shrink-0 mt-0.5" />
                                                            <div className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{renderCitationItem(attack)}</div>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        isChronLoading ? renderSubTabLoader() : deepResult?.chronology ? (
                                            <div className="space-y-6 relative border-l-2 border-gray-200 dark:border-gray-700 ml-4 pl-8 py-4">
                                                {deepResult.chronology.map((event: ChronologyEvent, i: number) => (
                                                    <div key={i} className="relative group bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover-lift">
                                                        ...
                                                        <div className="flex flex-col gap-2">
                                                            ...
                                                            <div className="text-gray-700 dark:text-gray-300 text-sm leading-relaxed">{renderCitationItem(event.event)}</div>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                        ...
                                        isContradictLoading ? renderSubTabLoader() : deepResult?.contradictions ? (
                                            <div className="grid gap-6">
                                                {deepResult.contradictions.length === 0 ? (
                                                    <div className="bg-white dark:bg-gray-800 p-12 rounded-[1.5rem] text-center border border-gray-200 dark:border-gray-700 shadow-sm">
                                                        ...
                                                        <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">Nuk u gjetën kontradikta mes deklaratave dhe provave.</p>
                                                    </div>
                                                ) : (
                                                        ...
                                                        <div key={i} className="bg-white dark:bg-gray-800 border border-warning-start/30 p-6 rounded-[1.5rem] shadow-lg shadow-warning-start/5">
                                                            ...
                                                            <div className="grid md:grid-cols-2 gap-6 mb-4">
                                                                <div className="p-5 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700">
                                                                    ...
                                                                    <div className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed italic">"{renderCitationItem(c.claim)}"</div>
                                                                </div>
                                                                <div className="p-5 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-700">
                                                                    ...
                                                                    <div className="text-sm text-gray-700 dark:text-gray-300 font-medium leading-relaxed">{renderCitationItem(c.evidence)}</div>
                                                                </div>
                                                            </div>
                                                            <div className="mt-4 p-4 bg-warning-start/5 rounded-xl border border-warning-start/10">
                                                                ...
                                                                <div className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{renderCitationItem(c.impact)}</div>
                                                            </div>
                                                        </div>
                                                    ))
                                                )}
          ...
          {/* Footer Actions */}
          <div className="px-8 py-5 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 flex flex-col sm:flex-row gap-4 justify-between items-center shrink-0">
              <button 
