...
const renderCitationItem = (item: any) => {
    if (typeof item === 'object' && item !== null && (item.law || item.title)) {
        ...
        return (
            <div className="flex flex-col gap-3 w-full">
                ...
                {body && (
                    <div className="text-gray-700 dark:text-text-secondary text-sm whitespace-pre-wrap leading-relaxed pl-5 border-l-2 border-border-main ml-0.5 mt-1">
    ...
    return (
        <span className="leading-relaxed whitespace-pre-wrap text-gray-900 dark:text-text-primary">
            {parts.map((part, i) => {
            ...
            <AnimatePresence>
                {show && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute top-full left-1/2 transform -translate-x-1/2 mt-3 w-56 p-4 bg-white dark:bg-surface text-[11px] font-medium text-gray-700 dark:text-text-secondary rounded-xl border border-border-main shadow-lawyer-dark z-[100] text-center leading-relaxed"
                    >
  ...
  const modalContent = (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4 sm:p-6" onClick={onClose}>
        <motion.div initial={{ scale: 0.98, opacity: 0, y: 10 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.98, opacity: 0, y: 10 }} className="w-full h-full sm:h-[90vh] sm:max-w-6xl bg-white dark:bg-surface border border-border-main rounded-2xl shadow-xl dark:shadow-lawyer-dark overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
          ...
          {isLoading ? (
             ...
             <>
                ...
                {/* Content Body with zoom control */}
                <div 
                        ...
                        {activeTab === 'legal' && (
                            <>
                                <div className="bg-white dark:bg-surface p-8 rounded-[1.5rem] border border-border-main shadow-sm dark:shadow-lawyer-light hover-lift">
                                    <h3 className="text-[11px] font-black text-gray-500 dark:text-text-muted uppercase tracking-widest mb-5 flex items-center gap-3">
                        ...
                        {activeTab === 'war_room' && (
                            <div className="h-full flex flex-col">
                                ...
                                <div className="flex flex-wrap gap-3 mb-8 shrink-0 pb-2">
                                    <button onClick={() => setWarRoomSubTab('strategy')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'strategy' ? 'bg-primary-start text-white shadow-accent-glow' : 'bg-white dark:bg-surface border border-border-main text-gray-600 dark:text-text-muted hover:text-gray-900 dark:hover:text-text-primary'}`}>
                                        <Target size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_strategy', 'Plani Strategjik')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('adversarial')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'adversarial' ? 'bg-danger-start text-white shadow-lg shadow-danger-start/30' : 'bg-white dark:bg-surface border border-border-main text-gray-600 dark:text-text-muted hover:text-danger-start'}`}>
                                        <Skull size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_adversarial', 'Simulimi i Palës')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('timeline')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'timeline' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-white dark:bg-surface border border-border-main text-gray-600 dark:text-text-muted hover:text-indigo-500'}`}>
                                        <Clock size={14} className="inline mr-2 -mt-0.5" /> {t('analysis.subtab_timeline', 'Kronologjia')}
                                    </button>
                                    <button onClick={() => setWarRoomSubTab('contradictions')} className={`px-5 py-2.5 rounded-xl text-[10px] uppercase tracking-widest font-black transition-all ${warRoomSubTab === 'contradictions' ? 'bg-warning-start text-white shadow-lg shadow-warning-start/30' : 'bg-white dark:bg-surface border border-border-main text-gray-600 dark:text-text-muted hover:text-warning-start'}`}>
                                    ...
                                    {warRoomSubTab === 'strategy' ? (
                                        <div className="space-y-8">
                                            <div className="bg-white dark:bg-surface p-8 rounded-[1.5rem] border border-border-main shadow-sm dark:shadow-lawyer-light">
                                                <h3 className="text-[11px] font-black text-gray-500 dark:text-text-muted uppercase tracking-widest mb-5 flex items-center gap-3"><Target size={16} className="text-primary-start"/> {t('analysis.section_analysis', 'Analiza Strategjike')}</h3>
