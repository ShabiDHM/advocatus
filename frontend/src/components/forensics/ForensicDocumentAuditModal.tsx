// FILE: frontend/src/components/forensics/ForensicDocumentAuditModal.tsx
// PHOENIX PROTOCOL - FORENSIC DOCUMENT AUDIT MODAL V1.0.1 (UNUSED IMPORT PURGED)
// ZERO TS WARNINGS • MIRROR OF StandardDocumentAuditModal • FORENSIC SERVICE INTEGRATION • 100% COMPLETE CODE

import React, { useState, useMemo, useRef, useEffect, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import {
  X, Copy, CheckCircle2,
  Loader2, Maximize2, Minimize2, Trash2, ZoomIn, ZoomOut, ArrowDown, Sparkles, Lock, ShieldCheck
} from 'lucide-react';
...
interface ForensicDocumentAuditModalProps {
  isOpen: boolean;
  onClose: () => void;
  caseId: string;
  documentId: string;
...
const markdownToWordHtml = (markdown: string): string => {
  ...
  return `
    ...
    <head>
      <meta charset="utf-8">
      <title>Pasqyra Forenzike e Shkresës</title>
      <style>
...
export const ForensicDocumentAuditModal: React.FC<ForensicDocumentAuditModalProps> = ({
  isOpen,
  onClose,
  caseId,
  documentId,
  ...
  useEffect(() => {
    if (isOpen && caseId && documentId) {
      setIsLoading(false);
      forensicDeskService.listForensicDocuments(caseId)
        .then((docs: any[]) => {
          const currentDoc = (docs || []).find((d: any) => String(d.id || d._id) === String(documentId));
          const savedAudit = currentDoc?.latest_analysis || currentDoc?.latest_forensic_audit || '';
  ...
  // GJENERIMI I AUDITIMIT DOKTRINAR TË SHKRESËS FORENZIKE
...
Rregull i Hekurt: Përgjigju me gjuhë zyrtare gjyqësore, me pika hierarkike, pa tabela të fryra dhe pa formalitete të tepërta hyrëse.`;
    ...
    try {
      const contextHeader = `Lënda Forenzike. Dokumenti i fokusuar: ${documentName}. Vula: AKTIVE`;
      ...
      if (finalContent.length > 0) {
        setIsSaving(true);
        try {
          await forensicDeskService.saveForensicDocumentAudit(caseId, documentId, finalContent);
        } catch (saveErr) {
          console.error("Forensic Document Audit Persist Error:", saveErr);
      ...
      console.error("Forensic Doc Audit Error:", err);
      alert("Ndodhi një gabim gjatë auditimit forenzik të shkresës.");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleClearContent = async () => {
    ...
    const confirmWipe = window.confirm("A jeni i sigurt që dëshironi të asgjësoni plotësisht pasqyrën forenzike të kësaj shkrese nga serveri (Total Cascade Wipeout)?");
    if (!confirmWipe) return;

    setIsPurging(true);
    try {
      await forensicDeskService.clearForensicDocumentAudit(caseId, documentId);
      setReportContent('');
    } catch (err) {
      console.error("Could not purge forensic document audit on MongoDB:", err);
      alert("Dështoi asgjësimi i pasqyrës forenzike të shkresës në server.");
    } finally {
      setIsPurging(false);
    }
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[250] p-2 sm:p-4 md:p-6 select-none">
        ...
        >
          ...
          >
            <style>{`
              .fast-forensic-audit p, .fast-forensic-audit li {
                font-size: ${activeFont.base}px !important;
                line-height: ${activeFont.line} !important;
              }
              .fast-forensic-audit h1, .fast-forensic-audit h2, .fast-forensic-audit h3 {
            ...
            {!reportContent && !isLoading ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6 sm:p-12 my-auto space-y-4">
                ...
                <div>
                  <h4 className="text-base font-bold text-text-primary">Ekzaminimi Forenzik i Shkresës</h4>
                  <p className="text-xs text-text-muted max-w-md mt-1">
                    Ekzaminoni këtë shkresë forenzike për vlefshmërinë e neneve, precedentët supremë dhe shkeljet procedurale.
                  </p>
                </div>
                <button
                  type="button"
                ...
                >
                  <Sparkles size={14} />
                  <span>Fillo Auditimin Forenzik</span>
                </button>
              </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 my-auto space-y-3">
                ...
                <p className="text-xs font-bold text-text-primary uppercase tracking-wider">
                  Duke audituar shkresën forenzike dhe verifikuar nenet...
                </p>
              </div>
            ) : (
              <div className="markdown-content fast-forensic-audit prose prose-slate dark:prose-invert max-w-none text-text-primary">
...
export default ForensicDocumentAuditModal;