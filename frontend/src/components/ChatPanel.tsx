...
// PHOENIX PROTOCOL - CLEAN BUILD V9.3
// 1. FIX: Removed unused 'moment' import.
...
const Dropdown: React.FC<DropdownProps> = ({ trigger, items, onSelect }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1.0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.1 }} className="absolute right-0 top-full mt-2 w-56 bg-[#1f2937] border border-glass-edge rounded-xl shadow-2xl z-50 overflow-hidden">
                        {items.map(item => <button key={item.id} onClick={() => handleSelect(item.id)} className="w-full text-left flex items-center gap-3 px-3 py-2 text-sm hover:bg-white/5 text-gray-200">{item.icon}<span className="truncate">{item.label}</span></button>)}
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
...
const ChatPanel: React.FC<ChatPanelProps> = ({ 
  ...
  return (
    <div className={`flex flex-col bg-background-dark rounded-2xl shadow-xl overflow-hidden ${className}`}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-glass-edge bg-background-light/50 backdrop-blur-md flex-shrink-0">
        ...
        <div className="flex items-center gap-1.5">
            <Dropdown items={contextItems} onSelect={setSelectedContextId} trigger={<div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-black/20 border border-white/5 hover:border-white/10 text-xs font-medium text-gray-300 transition-all max-w-[150px]">{selectedContextItem.icon}<span className="truncate">{selectedContextItem.label}</span><ChevronDown className="h-3 w-3 opacity-50 flex-shrink-0" /></div>} />
            <Dropdown items={jurisdictionItems} onSelect={(id) => setJurisdiction(id as Jurisdiction)} trigger={<div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-black/20 border border-white/5 hover:border-white/10 text-xs font-medium text-gray-300 transition-all">{selectedJurisdictionItem.icon}<span className="hidden sm:inline">{selectedJurisdictionItem.label}</span><ChevronDown className="h-3 w-3 opacity-50" /></div>} />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.length === 0 && !isSendingMessage ? (
            <div className="flex flex-col items-center justify-center h-full text-center opacity-50"><Brain size={48} className="mb-4 text-primary-start" /><p className="text-sm text-gray-400">{t('chatPanel.welcomeMessage')}</p></div>
        ) : (
        ...
        {isSendingMessage && <div className="flex items-start gap-2"><div className="w-8 h-8 rounded-full bg-background-light border border-glass-edge flex items-center justify-center flex-shrink-0"><Brain className="w-4 h-4 text-primary-start animate-pulse" /></div><div className="bg-white/5 text-gray-400 rounded-2xl rounded-bl-none px-4 py-3 text-sm flex items-center gap-2"><RefreshCw className="h-3 w-3 animate-spin" />{t('chatPanel.thinking')}</div></div>}
