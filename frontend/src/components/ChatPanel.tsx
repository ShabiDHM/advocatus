...
// PHOENIX PROTOCOL - CHAT PANEL V5.2 (RESTORED INTEGRITY)
// 1. FIX: Restored all missing Types and Interfaces (ChatPanelProps, ReasoningMode, etc.).
// 2. FIX: Resolved all TS build errors (TS2304, TS7006, TS6133).
// 3. FIX: Removed 'cleanChatText' to allow professional badge rendering.
// 4. RESTORED: All UI polish (Avatars, FileCheck badges, and Copy logic).
// 5. STATUS: 0 Build Errors. Professional UI + Senior Intelligence fully synchronized.
    ...
    return (
        <button 
            ...
            className={`absolute top-2 right-2 p-1.5 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100 ${
                ...
                isUser ? 'bg-white/10 text-white/70 hover:text-white hover:bg-white/20' : 
...
const MarkdownComponents = {
    ...
    a: ({href, children}: any) => {
        ...
        if (isDocLink) {
            ...
            const isEvidence = contentStr.includes("PROVA") || contentStr.includes("Dokument") || contentStr.includes("DOKUMENTI");
...
const ChatPanel: React.FC<ChatPanelProps> = ({ 
  ...
  useEffect(() => {
      if (!isPro && reasoningMode === 'DEEP') setReasoningMode('FAST');
  }, [isPro, reasoningMode]);
        ...
        <div className="flex items-center gap-2">
            <div className="flex items-center bg-black/30 rounded-lg p-0.5 mr-2 border border-white/5">
                <button
                    ...
                    className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold transition-all ${reasoningMode === 'FAST' ? 'bg-blue-500/20 text-blue-400' : 'text-gray-500 hover:text-gray-300'}`}
                >
                ...
                <button
                    ...
                    className={`flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold transition-all ${
                        ...
                        : !isPro ? 'text-gray-600 opacity-60 cursor-not-allowed' : 'text-gray-500 hover:text-gray-300'
                    }`}
                >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar z-0 relative min-h-0 bg-black/20">
        {messages.length === 0 && !isSendingMessage ? (
            ...
            messages.map((msg: ChatMessage, idx: number) => {
                ...
                // Do not use the legacy cleaner - let Markdown render the doc:// badges
                const displayContent = msg.content;
                if (isAi && !displayContent && isSendingMessage) return null;
                ...
                return (
                    <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex items-end gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`relative group max-w-[85%] sm:max-w-[75%] rounded-2xl px-5 py-3.5 text-sm leading-relaxed shadow-lg select-text ${
                            msg.role === 'user' ? 'bg-gradient-to-br from-primary-start to-primary-end text-white rounded-br-none' : 'glass-panel text-text-primary rounded-bl-none pr-10'
                        }`}>
                            <MessageCopyButton text={displayContent} isUser={msg.role === 'user'} />
      ...
      <div className="p-4 border-t border-white/5 bg-white/5 backdrop-blur-md z-10">
        <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="relative flex items-end gap-2">
            <textarea 
                ...
                rows={1} 
                className={`glass-input w-full pl-4 pr-12 py-3.5 rounded-xl text-sm resize-none custom-scrollbar transition-colors duration-300 ${reasoningMode === 'DEEP' ? 'border-purple-500/30' : ''}`} 
                style={{ maxHeight: '150px' }} 
            />
            <button 
                type="submit" 
                disabled={!input.trim() || isSendingMessage} 
                className={`absolute right-2 bottom-2 p-2 text-white rounded-lg transition-all ${
                    reasoningMode === 'DEEP' ? 'bg-gradient-to-r from-purple-500 to-indigo-600' : 'bg-gradient-to-r from-primary-start to-primary-end'
                }`}
            >
                <Send size={18} />
            </button>
