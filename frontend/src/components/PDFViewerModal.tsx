...
// PHOENIX PROTOCOL - PDF VIEWER V3.6 (SCROLLBAR CONSISTENCY)
// 1. UI: Applied consistent, custom scrollbar styles to the main viewer and thumbnail sidebar.
// 2. STATUS: Final UI polish complete.
...
const PDFViewerModal: React.FC<PDFViewerModalProps> = ({ documentData, caseId, onClose, onMinimizeRequest, t, directUrl, isAuth = false }) => {
                ...
                <div className="flex items-center justify-between w-full p-1 bg-[#101010] border-b border-white/10">
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="p-1">
                          <Download>
                            {(props) => (
                              <button className="rpv-toolbar__button" onClick={props.onClick} title={t('actions.download', 'Shkarko')}>
  ...
  const renderContent = () => {
    if (isLoading) return <div className="flex flex-col items-center justify-center h-full text-gray-400"><Loader className="animate-spin h-8 w-8 mb-2" /><span>{t('pdfViewer.loading', 'Duke hapur dokumentin...')}</span></div>;
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-0" onClick={onClose}>
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-[#1a1a1a] w-full h-full sm:max-w-6xl sm:max-h-[95vh] rounded-none sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-white/10" onClick={(e) => e.stopPropagation()}>
          <style>{`
            ...
            .rpv-default-layout__full-screen-target { z-index: 10001 !important; }

            /* PHOENIX FIX: Consistent Application Scrollbars */
            ...
            .rpv-default-layout__sidebar::-webkit-scrollbar-track {
                background: #1a1a1a;
            }
            .rpv-core__inner-pages::-webkit-scrollbar-thumb,
            .rpv-default-layout__sidebar::-webkit-scrollbar-thumb {
                background-color: #4b5563;
                border-radius: 4px;
            }
            .rpv-core__inner-pages::-webkit-scrollbar-thumb:hover,
            .rpv-default-layout__sidebar::-webkit-scrollbar-thumb:hover {
