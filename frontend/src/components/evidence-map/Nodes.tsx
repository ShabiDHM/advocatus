...
// PHOENIX PROTOCOL - NODES V2.1 (LINT CLEANUP)
// 1. FIX: Removed unused imports CheckCircle, XCircle, and Check (TS6133).
// 2. STATUS: Clean build ready. Professional hierarchy maintained.

import React, { memo } from 'react';
import { Handle, Position, NodeProps, Node, useReactFlow } from '@xyflow/react';
import { Gavel, FileText, Shield, Scale, Lock, ShieldCheck } from 'lucide-react'; // PHOENIX FIX: Cleaned imports
...
export const ClaimNode = memo(({ id, data, selected }: NodeProps<Node<MapNodeData>>) => {
  ...
  return (
    <div onDoubleClick={() => setNodes(nds => nds.map(n => n.id === id ? { ...n, data: { ...n.data, editing: true } } : n))} 
         className={`px-4 py-3 shadow-2xl rounded-xl bg-background-light border-2 transition-all ${baseBorder} border-l-4 ${data.isProven ? 'border-l-green-400' : 'border-l-yellow-400'} w-64`}>
      <div className="flex items-center pb-2 border-b border-white/10 mb-2 gap-2">
        <Gavel className="w-4 h-4 text-green-500" />
        <div className="text-[10px] font-black text-green-500 uppercase tracking-tighter">{t('evidenceMap.node.claimType')}</div>
      </div>
      <div className="text-sm font-bold text-white mb-1">{data.label}</div>
      <div className="text-xs text-text-muted italic line-clamp-3">{data.content}</div>
...
export const FactNode = memo(({ id, data, selected }: NodeProps<Node<MapNodeData>>) => {
  ...
  return (
    <div onDoubleClick={() => setNodes(nds => nds.map(n => n.id === id ? { ...n, data: { ...n.data, editing: true } } : n))}
         className={`px-4 py-3 shadow-2xl rounded-xl bg-background-light border-2 transition-all ${selected ? 'border-yellow-500 scale-105' : 'border-yellow-900/50'} border-l-4 border-l-yellow-500 w-60`}>
      <div className="flex items-center pb-2 border-b border-white/10 mb-2 gap-2">
...
export const LawNode = memo(({ id, data, selected }: NodeProps<Node<MapNodeData>>) => {
  ...
  return (
    <div onDoubleClick={() => setNodes(nds => nds.map(n => n.id === id ? { ...n, data: { ...n.data, editing: true } } : n))}
         className={`px-4 py-3 shadow-2xl rounded-xl bg-background-light border-2 transition-all ${selected ? 'border-purple-500 scale-105' : 'border-purple-900/50'} border-l-4 border-l-purple-500 w-60`}>
      <div className="flex items-center pb-2 border-b border-white/10 mb-2 gap-2">
...
export const EvidenceNode = memo(({ id, data, selected }: NodeProps<Node<MapNodeData>>) => {
  ...
  return (
    <div onDoubleClick={() => setNodes(nds => nds.map(n => n.id === id ? { ...n, data: { ...n.data, editing: true } } : n))}
         className={`px-4 py-3 shadow-2xl rounded-xl bg-background-light border-2 transition-all ${baseBorder} border-l-4 border-l-blue-500 w-64`}>
      <div className="flex items-center pb-2 border-b border-white/10 mb-2 gap-2">
        <FileText className="w-4 h-4 text-blue-500" />
        <div className="text-[10px] font-black text-blue-500 uppercase tracking-tighter">{t('evidenceMap.node.evidenceType')}</div>
      </div>
      <div className="text-sm font-bold text-white mb-1">{data.label}</div>
