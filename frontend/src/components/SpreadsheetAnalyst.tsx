...
// PHOENIX PROTOCOL - SPREADSHEET ANALYST V4.2 (DIRECT SCAN FIX)
// 1. UX FIX: 'Skano' button now triggers direct file/camera input instead of QR modal.
// 2. FEATURE: Added 'handleDirectScanUpload' to process images immediately via backend.
// 3. STATUS: Solves the "useless QR code" issue on mobile/desktop.
...
const SpreadsheetAnalyst: React.FC<SpreadsheetAnalystProps> = ({ caseId }) => {
    ...
    const directScanInputRef = useRef<HTMLInputElement>(null); // PHOENIX: Ref for hidden input
    ...
    // PHOENIX: New handler for Direct Scan
    ...
    // PHOENIX: Now only opens QR if specifically requested
    ...
    return (
        <div className="w-full h-full min-h-[500px] flex flex-col gap-6 p-2 sm:p-1">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
                        {!result && !isAnalyzing && (
                            <div className="flex gap-2 w-full md:w-auto">
                                ...
                                {/* PHOENIX: Main Scan Button - Triggers Camera directly */}
                                ...
                                {/* PHOENIX: QR Code Secondary Button */}
                                <button onClick={handleOpenQrModal} className="flex items-center justify-center p-2.5 rounded-xl border transition-colors cursor-pointer bg-black/20 border-gray-600 hover:border-white/50" title="Përdor pajisje tjetër (QR)">
            ...
            {/* PHOENIX: QR Code Modal (Now only shows if explicitly requested) */}
            <AnimatePresence>
                {showQrModal && (
