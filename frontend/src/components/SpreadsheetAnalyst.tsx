...
const SpreadsheetAnalyst: React.FC<SpreadsheetAnalystProps> = ({ caseId }) => {
    ...
    }, [result, chatHistory, file, fileName, caseId]);
    ...
    const runAnalysis = async (fileToAnalyze: File) => {
        ...
        try {
            ...
            setChatHistory([{
                id: 'init',
                role: 'agent',
                content: t('analyst.systemWelcome'),
                timestamp: new Date()
            }]);
        } catch (err: any) {
            console.error(err);
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-auto lg:h-[850px]">
                        ...
                        <div className="flex flex-col gap-6 overflow-visible lg:overflow-y-auto custom-scrollbar h-auto lg:h-full lg:pr-2">
                            <div className="glass-panel p-5 rounded-2xl border border-white/10 bg-white/5 flex flex-col shrink-0">
                             ...
                             {result.recommendations && result.recommendations.length > 0 && (
                                <div className="glass-panel p-4 rounded-xl border border-emerald-500/20 bg-emerald-900/10 shrink-0">
                                    <h3 className="text-sm font-bold text-emerald-400 mb-2 flex items-center gap-2"><Lightbulb className="w-4 h-4" />{t('analyst.recommendations')}</h3>
                                    <ul className="space-y-1">{result.recommendations.map((rec, i) => <li key={i} className="flex gap-2 items-start text-xs text-gray-300"><ArrowRight className="w-3 h-3 text-emerald-500 shrink-0 mt-0.5" /><span>{rec}</span></li>)}</ul>
                                </div>
                            )}
                            ...
                            <div className="glass-panel p-5 rounded-2xl border border-white/10 bg-white/5 flex flex-col shrink-0 min-h-[500px]">
                                <h3 className="text-md font-bold text-white mb-4 flex items-center gap-2 shrink-0"><AlertTriangle className="text-yellow-400 w-4 h-4" />{t('analyst.redFlags')}</h3>
                        ...
                        <div className="glass-panel rounded-2xl border border-primary-start/30 bg-black/40 flex flex-col h-[600px] lg:h-full overflow-hidden shadow-2xl relative sticky top-0">
                            ...
                            <div className="p-4 border-b border-white/10 bg-white/5 flex items-center gap-3 shrink-0">
                                ...
                                <div><h3 className="text-sm font-bold text-white">{t('analyst.agentTitle')}</h3></div>
                            </div>
                            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                                {chatHistory.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-gray-500 gap-2"><MessageSquare className="w-8 h-8 opacity-50" /><p className="text-xs">{t('analyst.consoleReady')}</p></div> : chatHistory.map((msg) => <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] rounded-2xl p-4 text-sm leading-relaxed break-words ${msg.role === 'user' ? 'bg-primary-start text-white rounded-br-none' : 'bg-white/10 text-gray-200 border border-white/5 rounded-bl-none'}`}><div>{renderMarkdown(msg.content)}</div>{msg.role === 'agent' && msg.evidenceCount !== undefined && <div className="mt-2 pt-2 border-t border-white/10 flex items-center gap-2 text-[10px] text-gray-400"><ShieldAlert className="w-3 h-3" />{t('analyst.verifiedAgainst', { count: msg.evidenceCount })}</div>}</div></div>)}
                                {isInterrogating && <div className="flex justify-start"><div className="bg-white/5 rounded-2xl rounded-bl-none p-4 border border-white/5 flex gap-1"><div className="w-2 h-2 bg-gray-500 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} /><div className="w-2 h-2 bg-gray-500 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} /><div className="w-2 h-2 bg-gray-500 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} /></div></div>}
                                <div ref={chatEndRef} />
                            </div>
                            <form onSubmit={handleInterrogate} className="p-4 border-t border-white/10 bg-white/5 flex gap-2 shrink-0">
                                ...
                                <button type="submit" disabled={!question.trim() || isInterrogating} className="p-3 bg-primary-start text-white rounded-xl hover:bg-primary-end disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><Send className="w-5 h-5" /></button>
                            </form>
                        </div>
                    </motion.div>
                )}
