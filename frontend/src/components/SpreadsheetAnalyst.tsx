...
// PHOENIX PROTOCOL - REFACTOR V1.7 (FONT CONSISTENCY)
// 1. FIX: Increased anomaly detail font size from text-xs to text-sm.
// 2. STATUS: Consistent with DepositionAnalyst UI.
        ...
        <div className="w-full h-full min-h-[500px] flex flex-col gap-6 p-1">
            <div className="glass-panel p-6 rounded-2xl border border-white/10 bg-white/5 relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Activity className="text-primary-start" />{t('analyst.title', 'Smart Financial Analyst')}</h2>
                        <p className="text-gray-400 mt-1 max-w-xl">{t('analyst.subtitle', 'Upload Excel/CSV records for automated forensic analysis.')}</p>
                    </div>
                ...
                {result && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 space-y-6">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {Object.entries(result.key_statistics).map(([key, value]) => (
                                    <div key={key} className="bg-white/5 p-4 rounded-xl border border-white/10 text-center hover:border-primary-start/30 transition-colors">
                                        <p className="text-xs text-gray-400 uppercase tracking-wider mb-1 font-bold">{formatStatKey(key)}</p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{result.charts.map(chart => <SimpleBarChart key={chart.id} chart={chart} />)}</div>
                        </div>
                                    ...
                                    {result.anomalies.length === 0 ? (
                                        ...
                                        result.anomalies.map((anomaly, idx) => (
                                            <div key={idx} className="p-4 bg-red-500/5 border border-red-500/20 rounded-xl hover:bg-red-500/10 transition-colors group">
                                                ...
                                                <p className="text-sm text-gray-400 leading-snug group-hover:text-gray-300 transition-colors">{anomaly.reason}</p>
                                            </div>
                                        ))
                                    )}
                                </div>
            ...
            <ArchiveImportModal isOpen={isArchiveOpen} onClose={() => setIsArchiveOpen(false)} caseId={caseId} mode="select" allowedExtensions={['xlsx', 'xls', 'csv']} onSelectFile={handleArchiveSelect} />
        </div>
    );
};
