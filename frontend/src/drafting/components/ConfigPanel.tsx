...
// PHOENIX PROTOCOL - CONFIG PANEL V7.7 (RESTORING CASE CONTEXT)
      ...
      <div className="flex flex-col gap-6 flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex-shrink-0">
          ...
          <div className="relative">
            ...
            <select
              className="w-full pl-11 pr-4 py-3.5 bg-surface border border-border-main rounded-xl text-sm font-bold text-text-primary focus:border-primary-start outline-none transition-all appearance-none cursor-pointer"
        ...
        <div className="flex-shrink-0 relative z-20">
          <div className="flex justify-between items-center mb-2">
            ...
            {!isPro && <span className="text-[9px] text-warning-start font-black bg-warning-start/10 px-2 py-0.5 rounded border border-warning-start/20 uppercase tracking-widest"><Lock size={10} /> PRO</span>}
          </div>
          <button ref={buttonRef} type="button" onClick={() => isPro && setIsOpen(!isOpen)} disabled={!isPro} className="w-full pl-11 pr-10 py-3.5 bg-surface border border-border-main rounded-xl text-sm font-bold text-text-primary flex items-center justify-between pointer-events-auto">
          ...
          {isOpen && isPro && (
            <div ref={dropdownRef} className="absolute z-[9999] mt-1 w-full bg-white dark:bg-gray-900 border border-border-main rounded-xl shadow-2xl max-h-60 overflow-y-auto custom-scrollbar">
              <div onClick={() => handleSelect('generic')} className="px-4 py-2 hover:bg-hover cursor-pointer text-sm font-bold text-text-primary">{t('drafting.templateGeneric')}</div>
              {templateGroups.map((group) => (
                <div key={group.label}>
                  <div className="px-4 py-1.5 text-xs font-black uppercase tracking-widest text-text-muted bg-gray-50 dark:bg-gray-800 sticky top-0 z-10 border-y border-border-main/50">{group.label}</div>
                  {group.options.map((opt) => (
                    <div key={opt} onClick={() => handleSelect(opt)} className="px-4 py-2 hover:bg-hover cursor-pointer text-sm font-bold text-text-primary pl-6">{getOptionLabel(opt)}</div>
                  ))}
                </div>
              ))}
            </div>
        ...
        {/* Textarea & Submit - Same as before */}
        <div className="flex-1 flex flex-col min-h-0">
           <textarea value={context} onChange={(e) => onChangeContext(e.target.value)} placeholder={placeholder} className="w-full p-5 bg-surface border border-border-main rounded-xl text-sm flex-1 resize-none font-medium text-text-primary focus:border-primary-start outline-none transition-all" />
        </div>

        <button onClick={() => onSubmit()} disabled={isSubmitting || !context.trim()} className="btn-primary w-full h-14 flex items-center justify-center gap-3 mt-2 flex-shrink-0 disabled:opacity-40">
          {isSubmitting ? <RefreshCw className="animate-spin" size={18} /> : <Send size={18} />}
          <span className="uppercase tracking-widest font-black text-xs">{isSubmitting ? t('drafting.statusWorking') : t('drafting.generateBtn')}</span>
        </button>
      </div>
    </div>
  );
