...
// 2. MODIFIED: All UI text now uses the t() function.

import { useState, useCallback, useEffect } from 'react';
import { useParams } from 'react-router-dom';
import { 
  ReactFlow, Background, Controls, applyEdgeChanges, applyNodeChanges, addEdge,
  Connection, Edge, Node, OnNodesChange, OnEdgesChange, OnConnect, Panel, MarkerType
} from '@xyflow/react';
...
const EvidenceMapPage = () => {
  ...
  const onConnect: OnConnect = useCallback(
    (params: Connection) => setEdges((eds) => addEdge({ 
      ...params, animated: true, type: 'smoothstep', markerEnd: { type: MarkerType.ArrowClosed }, style: { strokeWidth: 2 }
    }, eds)),
    []
  );
  ...
  const addNewNode = (type: 'claimNode' | 'evidenceNode') => {
    const newNode: Node = {
      ...
      data: { 
        label: type === 'claimNode' ? t('evidenceMap.node.newClaim') : t('evidenceMap.node.newEvidence'), 
  ...
  return (
    <div className="w-full h-[calc(100vh-64px)] bg-background-dark">
      <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} nodeTypes={nodeTypes} fitView colorMode="dark">
