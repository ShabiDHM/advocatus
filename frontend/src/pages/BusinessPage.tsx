...
// 1. FIX: Moved 'getFileIcon' to top-level scope so 'ArchiveCard' can use it.
// 2. FIX: Renamed Lucide 'File' to 'FileIcon' to avoid collision with TS 'File' type.
// 3. CLEANUP: Removed unused 'storageInfo' and imports.

import React, { useEffect, useState, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { 
    Building2, Mail, Phone, MapPin, Globe, Palette, Save, Upload, Loader2, 
    CreditCard, FileText, Plus, Download, Trash2, FolderOpen, File as FileIcon, // Renamed to avoid collision
...
const BusinessPage: React.FC = () => {
  ...
  // PHOENIX FIX: Added Handlers for Expense Actions
  const handleViewExpense = async (expense: Expense) => { 
      ...
      try { 
          ...
          setViewingUrl(url); 
          setViewingDoc({ id: expense.id, file_name: filename, mime_type: mime, status: 'READY' } as any); 
      } catch (error) { alert("Receipt not found"); } 
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6">
      ...
      {activeTab === 'finance' && (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
            ...
            {expenses.length > 0 && (<div className="bg-background-dark border border-glass-edge rounded-3xl p-6 shadow-xl mb-8"><h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><MinusCircle className="text-rose-500" size={20} /> {t('finance.expense')}</h3><div className="divide-y divide-white/5">{expenses.map(exp => (<div key={exp.id} className="bg-background-dark border border-glass-edge rounded-2xl p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-white/20 transition-all group"><div className="flex items-center gap-5 w-full md:w-auto overflow-hidden"><div className={`p-3.5 rounded-xl flex-shrink-0 bg-rose-500/10 text-rose-400`}><MinusCircle size={28} /></div><div className="min-w-0"><h3 className="font-bold text-white text-lg truncate pr-2">{exp.category}</h3><p className="text-sm text-gray-400 font-mono truncate">{exp.description} • {new Date(exp.date).toLocaleDateString()}</p>{exp.receipt_url && <div className="flex items-center gap-1 mt-1 text-xs text-indigo-400"><Paperclip size={10} /> <span>Faturë e bashkangjitur</span></div>}</div></div><div className="flex items-center justify-between w-full md:w-auto gap-4 md:gap-8 mt-2 md:mt-0"><div className="text-left md:text-right"><p className="text-xl font-bold text-white">-€{exp.amount.toFixed(2)}</p><span className="inline-block text-[10px] px-2.5 py-1 rounded-full font-bold tracking-wide uppercase mt-1 bg-rose-900/30 text-rose-400 border border-rose-500/20">EXPENSE</span></div><div className="flex gap-1 sm:gap-2"><button onClick={() => handleViewExpense(exp)} disabled={!exp.receipt_url} className={`p-2.5 rounded-xl transition-colors ${exp.receipt_url ? 'bg-white/5 hover:bg-white/10 text-gray-400 hover:text-blue-400' : 'bg-transparent text-gray-700 cursor-not-allowed'}`} title={t('archive.view')}>{openingDocId === exp.id ? <Loader2 className="h-4 w-4 animate-spin text-blue-400" /> : <Eye size={18} />}</button><button onClick={() => handleDownloadExpense(exp)} disabled={!exp.receipt_url} className={`p-2.5 rounded-xl transition-colors ${exp.receipt_url ? 'bg-white/5 hover:bg-white/10 text-gray-400 hover:text-green-400' : 'bg-transparent text-gray-700 cursor-not-allowed'}`} title={t('archive.download')}><Download size={18} /></button><button onClick={() => deleteExpense(exp.id)} className="p-2.5 bg-white/5 hover:bg-white/10 rounded-xl text-gray-400 hover:text-red-400 transition-colors" title={t('general.delete')}><Trash2 size={18} /></button></div></div></div>))}</div></div>)}
