...
import { 
    ...
    FolderInput, Paperclip, CheckCircle, Calculator, Edit2, Activity
} from 'lucide-react';
...
const BusinessPage: React.FC = () => {
  ...
  const handleViewExpense = async (expense: Expense) => { 
      if (!expense.receipt_url) { alert(t('error.noReceiptAttached')); return; }
      ...
      } catch (error) { alert(t('error.receiptNotFound')); } 
  ...
  const handleArchiveExpenseClick = (expenseId: string) => { 
      ...
      if (!expense || !expense.receipt_url) { alert(t('error.noReceiptToArchive')); return; }
  ...
  const handleEditInvoice = (invoice: Invoice) => {
      ...
      setNewInvoice({
          ...
          notes: invoice.notes || '',
          status: invoice.status // PHOENIX: Load existing status
      });
  ...
  const handleCreateOrUpdateInvoice = async (e: React.FormEvent) => {
      ...
      try {
          ...
          const payload = { 
              ...
              status: newInvoice.status // PHOENIX: Send status update
          };
  ...
  const closeInvoiceModal = () => {
      ...
      setNewInvoice({ client_name: '', client_email: '', client_phone: '', client_address: '', client_city: '', client_tax_id: '', client_website: '', tax_rate: 18, notes: '', status: 'DRAFT' });
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex gap-3 w-full sm:w-auto flex-wrap justify-end">
                    ...
                    <button onClick={() => setShowExpenseModal(true)} className="flex-1 sm:flex-none justify-center flex items-center gap-2 px-4 py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl shadow-lg transition-all font-medium whitespace-nowrap">
                        <MinusCircle size={20} /> <span>{t('finance.addExpense')}</span>
                    </button>
                    <button onClick={() => setShowInvoiceModal(true)} className="flex-1 sm:flex-none justify-center flex items-center gap-2 px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-lg transition-all font-medium whitespace-nowrap">
