...
// PHOENIX PROTOCOL - LAW ARTICLE READER V65.0 (RESTORED CLEAN ARTICLE ENGINE)
// 100% COMPLETE CODE • ZERO TS WARNINGS • ZERO TS2554 ERRORS • INSTANT RETRIEVAL
...
export default function LawArticlePage() {
  ...
  useEffect(() => {
    ...
    const loadArticleAndSyncMongo = async () => {
      ...
      try {
        const cleanArtParam = articleNumber.replace(/^neni\s*/i, '').replace(/\.$/, '').trim();
        const data = await lawService.getLawArticle(lawTitle, cleanArtParam);
        if (!isMounted) return;
        ...
        if (!chunkId) {
          chunkId = generateFallbackChunkId(lawTitle, cleanArtParam);
        }
        ...
        const updatedSourceInfo: SourceInfo = data.source_info || {
          ...
          matched_article: data.article_number || cleanArtParam,
        ...
        const mongoAnalysis = await lawService.getCachedLawAnalysis(lawTitle, cleanArtParam);
  ...
  const navigateToArticleNum = (targetArt: string) => {
    ...
    navigate(`/laws/article?lawTitle=${encodeURIComponent(lawTitle)}&articleNumber=${encodeURIComponent(clean)}`);
  };
  ...
  const rawArtNum = (article.article_number || articleNumber || '').replace(/^neni\s*/i, '').replace(/\.$/, '').trim();
