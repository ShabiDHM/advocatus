...
const EventDetailModal: React.FC<EventDetailModalProps> = ({ event, onClose, onUpdate }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[2000]">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-panel w-full max-w-2xl p-6 sm:p-8 rounded-[2rem] shadow-2xl border border-main bg-canvas overflow-hidden">
                    ...
                    <div className="flex items-start space-x-5">
                        ...
                        <div className="min-w-0">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <span className={`text-[11px] px-3 py-1 rounded-full font-bold uppercase tracking-wider ${style.text} bg-surface border ${style.border}`}>{t(`calendar.types.${event.event_type}`)}</span>
                                <span className={`text-[11px] px-3 py-1 rounded-full border border-main bg-surface text-text-secondary font-bold uppercase tracking-wider`}>{t(`calendar.priorities.${event.priority}`)}</span>
                            </div>
                        </div>
                    </div>
                    <button 
                ...
                <div className="flex flex-col-reverse sm:flex-row gap-3 mt-8 pt-6 border-t border-main">
                    <button type="button" onClick={onClose} className="w-full sm:w-auto px-6 h-11 rounded-xl text-sm font-semibold text-text-secondary hover:text-text-primary hover:bg-hover border border-main transition-colors focus:outline-none">{t('general.cancel', 'Anulo')}</button>
                    <button type="button" onClick={handleDelete} disabled={isDeleting} className="w-full sm:w-auto px-6 h-11 bg-danger-start/10 hover:bg-danger-start/20 text-danger-start border border-danger-start/20 rounded-xl font-bold text-sm transition focus:outline-none disabled:opacity-50">Fshij</button>
                </div>
            </motion.div>
        </div>
    );
...
const CreateEventModal: React.FC<CreateEventModalProps> = ({ cases, existingEvents, onClose, onCreate }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 z-[2000]">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel w-full max-w-lg max-h-[90vh] p-6 sm:p-8 rounded-[2.5rem] flex flex-col shadow-2xl border border-main bg-canvas overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-grow overflow-hidden">
                    <div className="overflow-y-auto pr-2 space-y-4 flex-grow custom-finance-scroll">
                        <div className="space-y-1.5"><label className="block text-[10px] font-bold text-primary-start uppercase tracking-widest ml-1">{t('calendar.createModal.relatedCase')}</label><select required value={formData.case_id} onChange={(e) => setFormData(prev => ({ ...prev, case_id: e.target.value }))} className="w-full px-4 h-11 bg-surface border border-main rounded-xl text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary-start/45"><option value="" className="bg-canvas">Zgjidhni rastin...</option>{cases.map(c => <option key={c.id} value={c.id} className="bg-canvas">{c.title || c.case_number}</option>)}</select></div>
                        <div className="space-y-1.5"><label className="block text-[10px] font-bold text-primary-start uppercase tracking-widest ml-1">{t('calendar.createModal.eventTitle')}</label><input type="text" required value={formData.title} onChange={(e) => setFormData(prev => ({ ...prev, title: e.target.value }))} className="w-full px-4 h-11 bg-surface border border-main rounded-xl text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary-start transition-all" /></div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5"><label className="block text-[10px] font-bold text-primary-start uppercase tracking-widest ml-1">Lloji</label><select value={formData.event_type} onChange={(e) => setFormData(prev => ({ ...prev, event_type: e.target.value as CalendarEvent['event_type'] }))} className="w-full px-4 h-11 bg-surface border border-main rounded-xl text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary-start/45">{Object.keys(t('calendar.types', { returnObjects: true }) as object).map(key => <option key={key} value={key} className="bg-canvas">{t(`calendar.types.${key}`)}</option>)}</select></div>
                            <div className="space-y-1.5"><label className="block text-[10px] font-bold text-primary-start uppercase tracking-widest ml-1">Prioriteti</label><select value={formData.priority} onChange={(e) => setFormData(prev => ({ ...prev, priority: e.target.value as CalendarEvent['priority'] }))} className="w-full px-4 h-11 bg-surface border border-main rounded-xl text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary-start/45">{Object.keys(t('calendar.priorities', { returnObjects: true }) as object).map(key => <option key={key} value={key} className="bg-canvas">{t(`calendar.priorities.${key}`)}</option>)}</select></div>
                        </div>
                        <div className="space-y-1.5"><label className="block text-[10px] font-bold text-primary-start uppercase tracking-widest ml-1">Data</label><DatePicker selected={eventDate} onChange={(date: Date | null) => setEventDate(date)} locale={currentLocale} dateFormat="dd.MM.yyyy" placeholderText="Klikoni për datën" className="w-full px-4 h-11 bg-surface border border-main rounded-xl text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary-start transition-all" portalId="react-datepicker-portal" required /></div>
                        
                        <div className="bg-surface border border-main rounded-2xl p-4 flex items-center justify-between cursor-pointer" onClick={() => setIsPublic(!isPublic)}><div className="flex items-center gap-4"><div className={`p-3 rounded-xl transition-colors ${isPublic ? 'bg-primary-start text-white animate-pulse' : 'bg-canvas text-text-secondary'}`}>{isPublic ? <Eye size={18} /> : <EyeOff size={18} />}</div><div><h4 className={`text-sm font-bold ${isPublic ? 'text-primary-start' : 'text-text-secondary'}`}>{isPublic ? 'Publike' : 'Private'}</h4><p className="text-[10px] text-text-secondary/50 uppercase tracking-widest">Për klientin</p></div></div><div className={`w-10 h-5 rounded-full relative transition-colors ${isPublic ? 'bg-primary-start' : 'bg-surface'}`}><div className={`absolute top-1 left-1 w-3 h-3 bg-white rounded-full transition-transform ${isPublic ? 'translate-x-5' : 'translate-x-0'}`} /></div></div>

                        {!showAdvanced && (
                            <button type="button" onClick={() => setShowAdvanced(true)} className="w-full text-[11px] font-bold text-text-secondary uppercase flex items-center justify-center gap-2 py-3 hover:text-text-primary transition-all focus:outline-none">
                        ...
                        {showAdvanced && (
                            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 pt-2">
                                <div className="space-y-1.5"><label className="block text-[10px] font-bold text-primary-start uppercase tracking-widest ml-1">Përshkrimi</label><textarea rows={3} value={formData.description} onChange={(e) => setFormData(prev => ({ ...prev, description: e.target.value }))} className="w-full p-4 bg-surface border border-main rounded-xl text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary-start transition-all resize-none" /></div>
                                <div className="space-y-1.5"><label className="block text-[10px] font-bold text-primary-start uppercase tracking-widest ml-1">Vendi</label><input type="text" value={formData.location} onChange={(e) => setFormData(prev => ({ ...prev, location: e.target.value }))} className="w-full px-4 h-11 bg-surface border border-main rounded-xl text-text-primary text-sm focus:outline-none focus:ring-2 focus:ring-primary-start transition-all" /></div>
                            </motion.div>
                        )}
                    </div>

                    <div className="flex flex-col-reverse sm:flex-row gap-3 pt-6 mt-auto border-t border-main">
                        <button type="button" onClick={onClose} className="w-full sm:w-auto px-6 h-11 rounded-xl text-sm font-semibold text-text-secondary hover:text-text-primary hover:bg-hover border border-main transition-all focus:outline-none">Anulo</button>
                        <button type="submit" disabled={isCreating} className="w-full sm:w-auto px-6 h-11 rounded-xl font-bold text-sm tracking-widest uppercase shadow-lg bg-primary-start hover:bg-opacity-95 text-white flex items-center justify-center gap-2 focus:outline-none shadow-primary-start/15 disabled:opacity-50">{isCreating ? <Loader2 className="animate-spin h-5 w-5 mx-auto" /> : 'Krijo'}</button>
                    </div>
                </form>
            </motion.div>
        </div>
...
const CalendarPage: React.FC = () => {
  ...
  // Apply scroll lock dynamically while mobile filters are active
  ...
  const renderMonthView = () => {
            ...
            {dayEvents.slice(0, 3).map(event => {
              ...
              return (
                <div key={eventId} className="relative w-full">
                    <button type="button" onClick={(e) => { e.stopPropagation(); setSelectedEvent(event); }} onMouseEnter={() => setHoveredEventId(eventId)} onMouseLeave={() => setHoveredEventId(null)} className={`w-full text-left px-2 py-1 rounded-lg border flex items-center gap-2 transition-all duration-300 ${style.bg} ${style.border} group-hover:shadow-lg focus:outline-none ${hoveredEventId === eventId ? 'scale-[1.05] z-50 ring-2 ring-white/10 shadow-black shadow-2xl' : ''}`}>
                    ...
                    <AnimatePresence>{hoveredEventId === eventId && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute left-0 bottom-full mb-3 z-[100] w-64 glass-panel p-4 rounded-2xl shadow-2xl pointer-events-none border border-main bg-canvas"><div className={`text-[10px] font-black uppercase mb-2 flex items-center gap-2 ${style.text}`}>{style.icon} {t(`calendar.types.${event.event_type}`)}</div><div className="text-text-primary font-bold text-sm mb-2">{event.title}</div><div className="text-text-secondary text-[11px] line-clamp-2 italic mb-3">{event.description || t('general.notAvailable')}</div><div className="pt-3 border-t border-main text-text-secondary text-[10px] flex justify-between font-bold"><span>{format(parseISO(event.start_date), 'HH:mm')}</span><span className="text-primary-start uppercase">{t(`calendar.priorities.${event.priority}`)}</span></div></motion.div>)}</AnimatePresence>
                </div>
              );
            })}
          </div>
    ...
    return (<div className="glass-panel flex-1 flex flex-col rounded-[2.5rem] overflow-hidden border border-main bg-canvas"><div className="grid grid-cols-7 bg-surface border-b border-main shrink-0">{weekDays.map(day => <div key={day} className="py-4 text-center text-[11px] font-black text-text-secondary uppercase tracking-widest">{day}</div>)}</div><div className="grid grid-cols-7 border-l border-t border-main flex-1 overflow-y-auto custom-finance-scroll bg-canvas">{days}</div></div>);
  };
  
  if (loading) return <div className="flex items-center justify-center h-[calc(100dvh-64px)] bg-canvas"><Loader2 className="animate-spin text-primary-start w-10 h-10" /></div>;
  ...
  const sidebarContent = (
    <div className="flex flex-col gap-8 min-h-0 bg-canvas">
      <div className="glass-panel flex-1 p-6 sm:p-8 rounded-[2.5rem] relative overflow-hidden flex flex-col border border-main bg-canvas">
        <h3 className="text-xs font-black text-text-primary mb-8 flex items-center gap-3 uppercase tracking-wider select-none"><Bell className="text-accent-start animate-bounce" size={16} />{t('calendar.upcomingAlerts')}</h3>
        <div className="flex-1 overflow-y-auto custom-finance-scroll space-y-6 pr-2">
          {upcomingAlerts.length === 0 ? (<div className="h-full flex items-center justify-center text-center px-4 italic text-text-secondary text-sm font-medium">S'ka afate.</div>) : (upcomingAlerts.map(ev => { const style = getEventStyle(ev.event_type, ev.category); return (<button type="button" key={getEventId(ev)} onClick={() => setSelectedEvent(ev)} className="w-full flex gap-5 items-start group text-left p-4 rounded-2xl hover:bg-hover transition-all border border-transparent hover:border-main focus:outline-none active:scale-95"><div className={`mt-2 w-2 h-2 rounded-full shrink-0 ${style.indicator} shadow-[0_0_12px_currentColor]`} /><div className="min-w-0 flex-1"><h4 className="text-sm font-bold text-text-secondary group-hover:text-primary-start transition-colors truncate tracking-tight">{ev.title}</h4><p className="text-[11px] text-text-secondary/50 mt-2 font-bold uppercase tracking-wider">{format(parseISO(ev.start_date), 'dd MMM')} • {t(`calendar.types.${ev.event_type}`)}</p></div></button>)}))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 gap-6 min-h-0 bg-canvas">
        ...
        <div className="shrink-0 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center justify-between sm:justify-start gap-4 w-full sm:w-auto h-auto sm:h-11">
            <div className="glass-panel flex items-center p-1 shrink-0 border border-main bg-surface h-11">
              ...
              <button type="button" onClick={() => navigateMonth('next')} className="flex items-center justify-center w-9 h-9 hover:bg-hover rounded-xl transition-all focus:outline-none"><ChevronRight size={18} className="text-text-secondary" /></button>
            </div>
            <div className="hidden sm:block"><h1 className="text-xl font-bold text-text-primary tracking-tight capitalize select-none">{format(currentDate, 'LLLL yyyy', { locale: currentLocale })}</h1></div>
          ...
          <button type="button" onClick={() => setIsCreateModalOpen(true)} className="btn-primary flex items-center justify-center gap-3 px-8 h-11 rounded-xl text-xs uppercase tracking-widest shrink-0 w-full sm:w-auto focus:outline-none">
        ...
        <div className="shrink-0 flex flex-col sm:flex-row gap-4 items-center h-auto sm:h-11">
          ...
          <div className="flex gap-4 h-11 w-full sm:w-auto">
            <button type="button" onClick={() => setShowFacts(!showFacts)} className={`flex items-center justify-center gap-3 px-6 h-11 rounded-xl text-xs font-bold uppercase tracking-wider transition-all focus:outline-none ${showFacts ? 'bg-primary-start text-white border-primary-start shadow-lg shadow-primary-start/15' : 'border border-main text-text-secondary hover:bg-hover'}`}><History size={14} /> {showFacts ? 'Gjithçka' : 'Afatet'}</button>
            <div className="glass-panel flex p-1 rounded-xl border border-main bg-surface h-11 items-center">
              <button type="button" onClick={() => setViewMode('month')} className={`px-5 h-9 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all focus:outline-none ${viewMode === 'month' ? 'bg-primary-start text-white shadow-md' : 'text-text-secondary hover:text-text-primary'}`}>Muaji</button>
              <button type="button" onClick={() => setViewMode('list')} className={`px-5 h-9 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all focus:outline-none ${viewMode === 'list' ? 'bg-primary-start text-white shadow-md' : 'text-text-secondary hover:text-text-primary'}`}>Lista</button>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-1 xl:grid-cols-4 gap-8 min-h-0 relative bg-canvas">
          <div className="xl:col-span-3 flex flex-col min-h-0 bg-canvas">{viewMode === 'list' ? renderListView() : renderMonthView()}</div>
          
          {/* Desktop sidebar (always visible on xl) */}
          <div className="hidden xl:flex xl:col-span-1 flex-col gap-8 min-h-0 bg-canvas">
            {sidebarContent}
          </div>
        </div>
      </div>
