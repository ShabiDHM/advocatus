...
// PHOENIX PROTOCOL - GATEKEEPER UI
// 1. LOGIC: On success, switches to a 'Pending Approval' view instead of redirecting.
// 2. UI: Uses professional icons (CheckCircle, Shield) to reassure the user.
...
const RegisterPage: React.FC = () => {
  ...
  const [isSuccess, setIsSuccess] = useState(false); // Controls the view
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Backend handles creation and sets is_active=False
      ...
      const msg = err.response?.data?.detail || t('auth.registerFailed', 'Regjistrimi dështoi.');
      setError(msg);
    } finally {
  ...
  // --- VIEW: SUCCESS / PENDING APPROVAL ---
  if (isSuccess) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-dark px-4">
        <div className="max-w-md w-full p-8 bg-background-light/10 backdrop-blur-md rounded-2xl border border-glass-edge shadow-2xl text-center transform transition-all duration-500 ease-in-out scale-100 opacity-100">
            ...
            <h2 className="text-2xl font-bold text-white mb-4">
                {t('auth.registrationSuccess', 'Llogaria u Krijua!')}
            </h2>
            
            <p className="text-text-secondary mb-8 leading-relaxed">
                {t('auth.gatekeeperMessage', 'Llogaria juaj është krijuar me sukses. Për arsye sigurie, ajo duhet të miratohet nga Administratori para se të mund të hyni.')}
            </p>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-8 flex items-start gap-3 text-left">
                ...
                <div>
                    ...
                    <p className="text-xs text-yellow-200/80">
                        {t('auth.approvalWait', 'Ju lutemi prisni njoftimin ose provoni të hyni pas pak.')}
                    </p>
                </div>
            </div>
            
            <Link to="/login" className="block w-full py-3 px-4 bg-white/10 hover:bg-white/20 border border-white/10 rounded-xl text-white font-semibold transition-all shadow-lg hover:shadow-xl">
    ...
    <div className="min-h-screen flex items-center justify-center bg-background-dark px-4">
      <div className="max-w-md w-full space-y-8 p-8 bg-background-light/10 backdrop-blur-md rounded-2xl border border-glass-edge shadow-2xl">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-text-secondary">{t('auth.registerSubtitle', 'Bashkohuni me platformën më të avancuar ligjore.')}</p>
        </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)}
                  className="block w-full pl-10 px-3 py-2 bg-background-dark/50 border border-glass-edge rounded-xl text-white placeholder-gray-500 focus:ring-2 focus:ring-primary-start focus:border-transparent outline-none"
          ...
          >
            {isSubmitting ? <Loader2 className="animate-spin h-5 w-5" /> : (
              <span className="flex items-center">
                {t('auth.registerButton', 'Regjistrohu')} <ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </span>
            )}
          </button>
        </form>
        <div className="text-center text-sm">
          ...
          <Link to="/login" className="font-medium text-primary-start hover:text-primary-end transition-colors">
            {t('auth.loginLink', 'Hyni këtu')}
          </Link>
        </div>
      </div>
    </div>
