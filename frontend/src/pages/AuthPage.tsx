...
const AuthPage: React.FC = () => {
  ...
  const validateInputs = (): boolean => {
    setError(null);
    if (!username || !password) { setError(t('auth.required')); return false; }
    if (!isLoginMode) {
      ...
      if (!email || !email.includes('@')) { setError(t('auth.invalidEmail')); return false; }
    }
    return true;
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen px-4 bg-background-dark bg-gradient-to-br from-background-dark via-background-light to-background-dark bg-[length:200%_200%] animate-gradient-shift">
      <motion.div 
