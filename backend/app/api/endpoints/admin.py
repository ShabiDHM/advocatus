...
# PHOENIX PROTOCOL - ADMIN ROUTER V5.0 (SYNC MODE)
# 1. FIX: Removed 'async/await' keywords.
# 2. LOGIC: Since 'admin_service' uses synchronous PyMongo, endpoints must be 'def' (not 'async def').
# 3. BENEFIT: FastAPI automatically threads these requests. Zero Pylance errors.
...
def get_all_organizations(
    ...
    """
    # Sync call - FastAPI handles threading
...
def upgrade_organization_tier(
    ...
    updated_org = admin_service.update_organization_tier(db, org_id, tier)
    if not updated_org:
