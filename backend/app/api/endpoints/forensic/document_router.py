...
# ==========================================================
# 3.1. AUDITIMI DOKTRINAR I SHKRESËS — PERSISTENCE (MULTI-DEVICE SYNC)
# ==========================================================
@router.post("/{case_id}/{doc_id}/audit", status_code=status.HTTP_200_OK)
def save_forensic_document_audit(
    case_id: str,
    doc_id: str,
    payload: ForensicAuditPayload,
    current_user: UserInDB = Depends(get_current_forensic_user),
    db: Database = Depends(get_db)
):
    """
    Ruan auditimin doktrinar të shkresës forenzike në MongoDB.
    Provon së pari koleksionin `forensic_documents`, pastaj `documents` si fallback.
    """
    user_id = str(current_user.id)
...
@router.post("/{case_id}/{doc_id}/clear-audit", status_code=status.HTTP_200_OK)
@router.delete("/{case_id}/{doc_id}/clear-audit", status_code=status.HTTP_200_OK)
def clear_forensic_document_audit(
    case_id: str,
    doc_id: str,
    current_user: UserInDB = Depends(get_current_forensic_user),
    db: Database = Depends(get_db)
    ...
    """
    Fshin auditimin e ruajtur të shkresës forenzike nga të dyja koleksionet e mundshme.
    """
    user_id = str(current_user.id)
    doc = None
    target_coll = FORENSIC_DOCS_COLLECTION
