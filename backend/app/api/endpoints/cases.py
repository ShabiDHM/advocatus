...
# PHOENIX PROTOCOL - CASES ROUTER V29.0 (SAAS ALIGNED BACKGROUND TASKS)
# 1. PIVOT: Switched from Celery to FastAPI BackgroundTasks for free-tier compatibility.
# 2. STATUS: No background worker required / 0% extra RAM footprint.
...
async def update_case_chat_history(case_id: str, update: ChatHistoryUpdate, current_user: Annotated[UserInDB, Depends(get_current_user)], db: Database = Depends(get_db)):
    case_oid = validate_object_id(case_id)
    case = await asyncio.to_thread(case_service.get_case_by_id, db=db, case_id=case_oid, owner=current_user)
    if not case: raise HTTPException(status_code=404, detail="Case not found")
    
    chat_history_dicts = []
    for msg in update.chat_history:
        msg_dict = msg.model_dump() if hasattr(msg, "model_dump") else msg.dict()
        if isinstance(msg_dict.get("timestamp"), datetime): msg_dict["timestamp"] = msg_dict["timestamp"].isoformat()
        chat_history_dicts.append(msg_dict)
    
    await asyncio.to_thread(db.cases.update_one, {"_id": case_oid}, {"$set": {"chat_history": chat_history_dicts}})
...
# PIVOTED: Using BackgroundTasks and Injecting Redis Client
...
async def upload_document_for_case(
    ...
    # PHOENIX PIVOT: Bypass Celery / Execute safely in background thread of the web process
...
async def bulk_delete_documents(case_id: str, current_user: Annotated[UserInDB, Depends(get_current_user)], document_ids: List[str] = Body(..., embed=True), db: Database = Depends(get_db), redis_client: redis.Redis = Depends(get_sync_redis)):
    validate_object_id(case_id)
    result = await asyncio.to_thread(document_service.bulk_delete_documents, db=db, redis_client=redis_client, document_ids=document_ids, owner=current_user)
    return {"deleted_count": result.get("deleted_count", 0), "deleted_finding_ids": result.get("deleted_finding_ids", [])}
