...
# PHOENIX PROTOCOL - DRAFTING ENDPOINT V2.1 (AGENTIC DISPATCHER)
# 1. LOGIC: No change required. This endpoint's only job is to dispatch the Celery task.
# 2. CLARITY: Added comments to reflect that the heavy lifting is now done by the Agent in the background.
...
async def create_drafting_job(
    ...
    """
    Receives a drafting request and dispatches it to the Agentic AI
    via a Celery background task. Returns immediately with a job ID.
    """
    try:
        task = celery_app.send_task(
            ...
            kwargs={
                ...
                "use_library": request_data.use_library # Now handled by the agent's logic
            }
        )
        job_id = task.id
        logger.info(f"Dispatched AGENTIC drafting job with Celery Task ID: {job_id}")
...
async def get_job_result(
    ...
    if result_doc.get("status") == "FAILED":
        raise HTTPException(status_code=500, detail=result_doc.get("error", "Drafting job failed."))

    final_text = result_doc.get("result_text") or result_doc.get("document_text")
    
    if not final_text:
        logger.error(f"❌ Document found but TEXT is missing for job {job_id}.")
