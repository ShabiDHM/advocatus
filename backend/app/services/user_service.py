...
# SECURITY HARDENING: Added comprehensive role normalization and validation
...
def _normalize_role(role: str) -> str:
    """Normalize role to uppercase and validate against allowed values"""
    ...
    if normalized_role in ['USER', 'ADMIN']:
        ...
        logger.warning(f"Invalid role '{role}' detected, normalizing to 'USER'")
        return 'USER'

def _ensure_user_data_integrity(user_data: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure user data integrity by normalizing critical fields"""
    if 'role' in user_data:
        user_data['role'] = _normalize_role(user_data['role'])
    
    # Ensure subscription_status is uppercase if present
    if 'subscription_status' in user_data and isinstance(user_data['subscription_status'], str):
        user_data['subscription_status'] = user_data['subscription_status'].upper()
...
def get_user_by_username(db: Database, username: str) -> Optional[UserInDB]:
    ...
    if not user_data:
        return None
    try:
        # Apply data integrity normalization before validation
        user_data = _ensure_user_data_integrity(user_data)
...
def update_user_profile(db: Database, user_id: ObjectId, data: Dict[str, Any]) -> Optional[UserInDB]:
    ...
    normalized_data = _ensure_user_data_integrity(data.copy())
...
def fix_user_role_integrity(db: Database, user_id: ObjectId) -> bool:
    """Fix role integrity for a specific user by ensuring role is uppercase"""
    user_data = db[USER_COLLECTION].find_one({"_id": user_id})
    if not user_data:
        return False
    
    current_role = user_data.get('role')
    if current_role and isinstance(current_role, str):
        ...
        if normalized_role != current_role:
            ...
            logger.info(f"Fixed role for user {user_id}: '{current_role}' -> '{normalized_role}'")
            return result.modified_count > 0
