...
# PHOENIX PROTOCOL - OCR ENGINE V9.0 (HIGH-DEFINITION 150 DPI SCANNED PDF PROCESSOR)
...
def _process_single_page_safe(doc_path: str, page_num: int) -> str:
    ...
    try:
        with fitz.open(doc_path) as doc:
            ...
            # Step 2: Scanned PDF page -> Convert to High-Res Image for OCR
            logger.info(f"🔍 [TextExtraction] Page {page_num + 1}: Scanned page detected. Running HD OCR...")
            ...
            # Matrix(2, 2) = 150 DPI Crisp HD rendering for fine legal print
...
def _extract_text_from_pdf(file_path: str) -> str:
    try:
        ...
        logger.info(f"⚡ [OCR Engine V9.0] Starting HD PDF Extraction. Total Pages: {total}")
        ...
        logger.info(f"🎉 [OCR Engine V9.0] Total Document Text Extracted: {len(full_result)} characters")
        return full_result
    except Exception as e:
