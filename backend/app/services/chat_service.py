...
# PHOENIX PROTOCOL - CHAT SERVICE V25.2 (TRUE STREAMING)
# 1. FIX: Replaced buffered 'await' with true 'async for' iteration in DEEP mode.
# 2. FIX: Hardened FAST mode prompt to ensure law citations are preserved.
# 3. STATUS: Streaming integrity confirmed across all modes.
...
async def stream_chat_response(
    ...
    try:
        ...
        if not mode or mode.upper() == 'FAST':
            ...
            # Senior Partner Prompt for Fast mode
            ...
            # True streaming from the generator
            async for token in agent_service.chat(query=user_query, user_id=user_id, case_id=case_id, document_ids=[document_id] if document_id else None):
                full_response += token
                yield token
