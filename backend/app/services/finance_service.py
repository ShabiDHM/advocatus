...
# PHOENIX PROTOCOL - FINANCE LOGIC V4 (EDIT SUPPORT)
# 1. ADDED: update_invoice (Recalculates totals automatically).
# 2. ADDED: update_expense.
# 3. FIX: Absolute imports for models.
...
class FinanceService:
    ...
    def update_invoice(self, user_id: str, invoice_id: str, update_data: InvoiceUpdate) -> InvoiceInDB:
        ...
        if "items" in update_dict or "tax_rate" in update_dict:
            ...
            # Convert dict items back to objects if needed (Pydantic handles this usually, but safe check)
