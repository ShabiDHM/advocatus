...
# PHOENIX PROTOCOL - FINANCE LOGIC
# 1. CALCULATION: Automatically computes totals and taxes.
# 2. NUMBERING: Auto-generates sequential invoice numbers (INV-001).
...
class FinanceService:
    ...
    def _generate_invoice_number(self, user_id: str) -> str:
        """Generates simple sequential numbers per user (INV-001, INV-002)."""
    ...
    def create_invoice(self, user_id: str, data: InvoiceCreate) -> InvoiceInDB:
        ...
        # Update line item totals just in case
        ...
        return InvoiceInDB(**invoice_doc, _id=result.inserted_id)
    ...
    def get_invoices(self, user_id: str) -> list[InvoiceInDB]:
        ...
        return [InvoiceInDB(**doc, _id=doc["_id"]) for doc in cursor]
    ...
    def get_invoice(self, user_id: str, invoice_id: str) -> InvoiceInDB:
        ...
        return InvoiceInDB(**doc, _id=doc["_id"])
    ...
    def update_invoice_status(self, user_id: str, invoice_id: str, status: str) -> InvoiceInDB:
        ...
        if not result:
            raise HTTPException(status_code=404, detail="Invoice not found")
        return InvoiceInDB(**result, _id=result["_id"])