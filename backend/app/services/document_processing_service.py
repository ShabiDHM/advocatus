...
def _stringify_metadata(metadata: Dict[str, Any]) -> Dict[str, Any]:
    """
    Recursively convert all metadata values to strings.
    This is a **defensive** measure – ensures ChromaDB never receives a non‑scalar.
    """
    result = {}
...
async def orchestrate_document_processing_mongo(db: Database, redis_client: redis.Redis, document_id_str: str):
    ...
    try:
        ...
        # --- PHOENIX CRITICAL: Force `case_number` to be a plain string ---
        if "case_number" in extracted_metadata:
            if isinstance(extracted_metadata["case_number"], dict):
                # Try to extract the most common field names
                case_num = (
        ...
        # --- Also force any other potential non‑scalar fields to strings ---
        ...
        # --- HYDRA: Async Summary Generation ---
        ...
        async def task_embeddings():
            ...
            for chunk in enriched_chunks:
                ...
                # PHOENIX: Ensure case_number is string (again, just to be safe)
